...
if 'directions' in config:
    nang = config['directions']
if 'frequencies' in config:
    nfre = config['frequencies']

print("grid = " , grid)
print("nang = " , nang)
print("nfre = " , nfre)
print("bathymetry = ", bathymetry)

timestep = duration_seconds(config['timestep'])
print("timestep = ",timestep,"s (",duration_str(seconds=timestep),")")


begoffo = config['forcing']['at'][1]['begin']
...
print("parameters name = ",config['output']['parameters']['name'] )
params = [ parameters[key]['index'] if isinstance(key, str) else parameters[f'{key:03d}']['index'] for key in config['output']['parameters']['name']  ]
print("parameters index = ",params )

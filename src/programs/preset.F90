...
!        ------------------
      ...
      IF (.NOT. VARS_4D%LALLOC) CALL VARS_4D%ALLOC(VARS_4D,UBOUNDS=[NPROMA_WAM,NANG,NFRE,NCHNK])

      IF (.NOT. FF_NOW%LALLOC) CALL FF_NOW%ALLOC(FF_NOW,UBOUNDS=[NPROMA_WAM,NCHNK]) 
      ...
      IF (.NOT. FF_NEXT%LALLOC) CALL FF_NEXT%ALLOC(FF_NEXT,UBOUNDS=[NPROMA_WAM,NCHNK])

      IF (.NOT. NEMO2WAM%LALLOC) CALL NEMO2WAM%ALLOC(NEMO2WAM,UBOUNDS=[NPROMA_WAM,NCHNK])
...
!$OMP END PARALLEL DO
      ...
      ELSE
        ...
        LLINIALL=.FALSE.
        LLOCAL=.TRUE.
        IF(.NOT. FIELDG%LALLOC) CALL FIELDG%ALLOC(FIELDG,LBOUNDS=[NXFFS, NYFFS], UBOUNDS=[NXFFE, NYFFE])

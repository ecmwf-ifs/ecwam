...
! ----------------------------------------------------------------------
     ...
     &            TH       ,COSTH    ,SINTH    ,FRIC
      ...
      USE YOWPCONS , ONLY : ZPI      ,G        ,EPSMIN, NPMAX
...
#include "sthq.intfb.h"
      ...
      REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(IN) :: WSWAVE, WDWAVE 
      REAL(KIND=JWRB), DIMENSION(KIJL,NANG), INTENT(IN) :: COSWDIF
      REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(IN) :: ESWELL,  FSWELL, THSWELL
      REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(IN) :: FSEA
      REAL(KIND=JWRB), DIMENSION(KIJL,NANG,NFRE), INTENT(INOUT) :: FLSW, SWM
      ...
      INTEGER(KIND=JWIM), DIMENSION(KIJL) :: NPEAK, NPK, NPKNA
      INTEGER(KIND=JWIM), DIMENSION(KIJL) :: FRINVMIJ
      INTEGER(KIND=JWIM), DIMENSION(KIJL) :: MMIN, MMAX
      INTEGER(KIND=JWIM), DIMENSION(KIJL) :: IPNOW
      INTEGER(KIND=JWIM), DIMENSION(KIJL,NTRAIN) :: IENERGY
      INTEGER(KIND=JWIM), DIMENSION(KIJL,NPMAX) :: NFRP, NTHP
      ...
      REAL(KIND=JWRB), DIMENSION(KIJL) :: ENEX, SUMETRAIN, SUMENE
      ...
      LOGICAL :: LLEPSMIN
      LOGICAL :: LLADDPART
      LOGICAL, DIMENSION(KIJL,NTRAIN) :: LPWSECTOR
      LOGICAL, DIMENSION(KIJL,NANG) :: LLCOSDIFF
...
!     PARTITION OF THE UNASSIGNED SWELL SPECTRA
      ...
      LLEPSMIN = .FALSE.
      CALL SEMEAN(SUNASGN, KIJS, KIJL, EUNASNG, LLEPSMIN)
      CALL STHQ(KIJS, KIJL, SUNASGN, THUNASNG)

      DO IJ=KIJS,KIJL
         IF ( EUNASNG(IJ) > SUMENE(IJ) ) THEN
           NPKNA(IJ)=NTRAIN
         ELSE
           NPKNA(IJ)=NTRAIN-1
         ENDIF
...
!     IF LESS THAN NPKNA PARTITIONS, CREATE A NEW PARTITION WITH THE UNASSIGNED SWELL SPECTRUM
      DO IJ=KIJS,KIJL
        IF (NPEAK(IJ) < NPKNA(IJ) .AND. EUNASNG(IJ) > 0.0_JWRB ) THEN
          NPEAK(IJ) = NPEAK(IJ)+1
...
!     6.1 Distribute extra energy proportionally to swell trains if more systems than NPKNA

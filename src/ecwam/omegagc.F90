...
REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(IN) :: UST
INTEGER(KIND=JWIM), DIMENSION(KIJL), INTENT(OUT) :: NS ! index in array XK_GC corresponding to XKS and OMS
REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(OUT) :: XKS   ! cut-off wave number
REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(OUT) :: OMS   ! cut-off angular frequency

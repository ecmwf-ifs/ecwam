...
!
      ...
      SUBROUTINE MEANSQS(XKMSS, KIJS, KIJL, F, WAVNUM, USTAR, WSWAVE, COSWDIF, XMSS)
...
!       *CALL* *MEANSQS (XKMSS, KIJS, KIJL, F, WAVNUM, USTAR, WSWAVE, COSWDIF, XMSS)*
...
#include "meansqs_lf.intfb.h"
      ...
      REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(IN) :: USTAR
      REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(IN) :: WSWAVE 
      ...
      REAL(KIND=JWRB), DIMENSION(KIJL) :: XMSSLF, XMSS_TAIL
      REAL(KIND=JWRB), DIMENSION(KIJL) :: HALP, FRGC
...
!     CALCULATE CONTRIBUTIONS TO MSS BEYOND MODEL CUT-OFF
!     CHECK IF USER HAS SET XKMSS = 0 INDICATING NO CONTRIBUTIONS
!     THIS FUNCTIONALITY ALLOWS ONLY MODEL SPECTRUM DOMAIN CONTRIBUTION TO BE
!     SPECIFIED EASILY WITHOUT CALCULATING THE MODEL MAXIMUM WAVENUMBER.
      IF (XKMSS /= 0._JWRB) THEN
        
!       CALCULATE INDEX TO WHICH TO CALCULATE MSS FROM SPECTRUM
        FCUT = SQRT(G*XKMSS)/ZPI
        NFRE_MSS = INT(LOG(FCUT/FR(1))/LOG(FRATIO))+1 
        NFRE_EFF = MIN(NFRE,NFRE_MSS)

!       COMPUTE THE PHILLIPS PARAMETER
        CALL HALPHAP(KIJS, KIJL, WAVNUM, WSWAVE, COSWDIF, F, HALP)
...
!       ADD TAIL CORRECTION TO MEAN SQUARE SLOPE (between FR(NFRE_EFF) and FRGC).
      ...
      XMSS(KIJS:KIJL) = XMSS(KIJS:KIJL) + XMSSLF(KIJS:KIJL)

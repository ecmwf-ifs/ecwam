...
!
      ...
      LOGICAL :: LWNEMOCOUIBR=.FALSE.
      LOGICAL :: LWNEMOCOUSTK=.FALSE.
      LOGICAL :: LWNEMOCOUWRS=.FALSE.
      ...
      PROCEDURE(IO_SERV_WV_OUTWSPEC), POINTER :: IO_SERV_WV_OUTWSPEC_HANDLER=> NULL()
      PROCEDURE(IO_SERV_WV_OUTINT)  , POINTER :: IO_SERV_WV_OUTINT_HANDLER  => NULL()
...
!      *LWNEMOCOUWRS  LOGI.  SEND ICE WAVE RADIATIVE STRESS TO NEMO
...
!      *LWNEMOCOUIBR LOGI.   USE THE ICE BREAKUP FIELD FROM NEMO for open ocean points as determined by the lake cover (see getcurr)
...
!      *LWCOUAST*  LOGICAL   WHEN COUPLED, IF TRUE THEN THE INPUT ATMOSPHERIC SURFACE STRESS OVER THE OCEAN WILL BE USED TO DETERMINE
!                            THE OCEAN SIDE STRESS (TAUOC).
...
CONTAINS
      ...
      SUBROUTINE IO_SERV_WV_OUTWSPEC (IJS, IJL, SPEC, MARSTYPE, CDATE, IFCST)
      USE PARKIND_WAVE, ONLY : JWIM, JWRB
      ...
      IF (.NOT.ASSOCIATED(IO_SERV_WV_OUTWSPEC_HANDLER)) CALL WAM_ABORT('IO_SERV_WV_OUTWSPEC_HANDLER IS NOT INITIALIZED')
      CALL IO_SERV_WV_OUTWSPEC_HANDLER (IJS, IJL, SPEC, MARSTYPE, CDATE, IFCST)

      END SUBROUTINE IO_SERV_WV_OUTWSPEC

      !----------------------------------------------------------------------------------------

      SUBROUTINE IO_SERV_WV_OUTINT (NIPRMOUT, NINFOBOUT, BOUT, INFOBOUT, MARSTYPE, CDATE, IFCST)
      USE PARKIND_WAVE, ONLY : JWIM, JWRB
      USE YOWGRID     , ONLY : NPROMA_WAM, NCHNK
      USE YOWABORT,     ONLY : WAM_ABORT
      IMPLICIT NONE
      INTEGER(KIND=JWIM),  INTENT(IN) :: NIPRMOUT
      INTEGER(KIND=JWIM),  INTENT(IN) :: NINFOBOUT
      REAL(KIND=JWRB),     INTENT(IN) :: BOUT(NPROMA_WAM, NIPRMOUT, NCHNK)
      INTEGER(KIND=JWIM),  INTENT(IN) :: INFOBOUT(NIPRMOUT, NINFOBOUT)
      CHARACTER(LEN=2),    INTENT(IN) :: MARSTYPE
      CHARACTER(LEN=14),   INTENT(IN) :: CDATE
      INTEGER (KIND=JWIM), INTENT(IN) :: IFCST

      IF (.NOT.ASSOCIATED(IO_SERV_WV_OUTINT_HANDLER)) CALL WAM_ABORT('IO_SERV_WV_OUTINT_HANDLER IS NOT INITIALIZED')
      CALL IO_SERV_WV_OUTINT_HANDLER (NIPRMOUT, NINFOBOUT, BOUT, INFOBOUT, MARSTYPE, CDATE, IFCST)

      END SUBROUTINE IO_SERV_WV_OUTINT

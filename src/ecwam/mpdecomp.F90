...
CALL WVOPENSUBBATHY (IREAD, NPR, KFILE_HANDLE_SUBGRID, KGRIB_HANDLE_SUBGRID)
WRITE(IU06,*) ''
...
IF ( .NOT. LLUNSTR ) THEN
  ...
  IF ( KGRIB_HANDLE_SUBGRID > 0 ) THEN
    ! FROM GRIB INPUT
    CALL GETGRBOBSTRCT(BLK2GLO, BLK2LOC, IREAD, NPR, KFILE_HANDLE_SUBGRID, KGRIB_HANDLE_SUBGRID)
  ELSE
    ! FROM BINARY INPUT

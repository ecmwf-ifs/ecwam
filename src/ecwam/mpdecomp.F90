...
#include "wam_nproma.intfb.h"
      ...
      INTEGER(KIND=JWIM) :: KFILE_HANDLE_SUBGRID, KGRIB_HANDLE_SUBGRID 
...
IF (IRANK == IREAD .AND. .NOT. LLUNSTR ) THEN
  CALL WVOPENSUBBATHY (KFILE_HANDLE_SUBGRID, KGRIB_HANDLE_SUBGRID)
  WRITE(IU06,*) ''
  WRITE(IU06,*) ' WAVE MODEL SUBGRID INFORMATION AVAILABLE ', KFILE_HANDLE_SUBGRID, KGRIB_HANDLE_SUBGRID
  CALL FLUSH (IU06)
ELSE
  KFILE_HANDLE_SUBGRID = -99
  KGRIB_HANDLE_SUBGRID = -99
ENDIF
...
IF ( .NOT. LLUNSTR ) THEN
  ...
  IF ( KGRIB_HANDLE_SUBGRID > 0 ) THEN
    ! FROM GRIB INPUT
    CALL GETGRBOBSTRCT(BLK2GLO, BLK2LOC, NPR, KFILE_HANDLE_SUBGRID, KGRIB_HANDLE_SUBGRID)
  ELSE
    ! FROM BINARY INPUT

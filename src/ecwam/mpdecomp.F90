...
SUBROUTINE MPDECOMP(NPR, MAXLEN, LLIRANK, LLWVENVI, LLNO_WAM_NPROMA)
...
#include "wam_nproma.intfb.h"
      ...
      LOGICAL, OPTIONAL, INTENT(IN) :: LLNO_WAM_NPROMA
...
! When redistributing the restart file to different number of MPI ranks
! nproma_wam must not change, hence the LLNO_WAM_NPROMA flag is used to control the call to wam_nproma
IF (PRESENT(LLNO_WAM_NPROMA)) THEN
  IF (.NOT. LLNO_WAM_NPROMA) THEN
    CALL WAM_NPROMA(IJS, IJL, MTHREADS, NPROMA_WAM)
  ENDIF
ELSE
  CALL WAM_NPROMA(IJS, IJL, MTHREADS, NPROMA_WAM)
ENDIF
...
IF (BLK2LOC%LALLOC) CALL BLK2LOC%DEALLOC(BLK2LOC)
CALL BLK2LOC%ALLOC(BLK2LOC, UBOUNDS=[NPROMA_WAM, NCHNK])
...
ELSE
  ...
  IF (LLWVENVI) THEN
    IF (WVENVI%LALLOC) CALL WVENVI%DEALLOC(WVENVI)
    CALL WVENVI%ALLOC(WVENVI,UBOUNDS=[NPROMA_WAM, NCHNK])
  ENDIF
...
#ifdef OMPGPU
!$omp target enter data map(to:KLON, KLAT, KCOR, WLAT, WCOR)
!$omp target enter data map(NTOPELST, NTOPE, IJTOPE, NFROMPELST, NFROMPE, NIJSTART)
#else

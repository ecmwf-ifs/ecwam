...
#include "wam_nproma.intfb.h"
      ...
      INTEGER(KIND=JWIM) :: KGRIB_HANDLE_SUBGRID 
...
CALL WVOPENSUBBATHY (KGRIB_HANDLE_SUBGRID)
WRITE(IU06,*) ''
WRITE(IU06,*) ' WAVE MODEL SUBGRID INFORMATION AVAILABLE ',KGRIB_HANDLE_SUBGRID
CALL FLUSH (IU06)
...
IF ( .NOT. LLUNSTR ) THEN
  ...
  IF ( KGRIB_HANDLE_SUBGRID > 0 ) THEN
    ! FROM GRIB INPUT
    CALL GETGRBOBSTRCT(BLK2GLO, BLK2LOC, NPR, KGRIB_HANDLE_SUBGRID)
  ELSE
    ! FROM BINARY INPUT

...
!     IFS IO SERVER ENABLED
      ...
      INTEGER(KIND=JWIM) :: KDELWI
      INTEGER(KIND=JWIB) :: IDURAT
      ...
      INTEGER(KIND=JWIB) :: IFCST
      INTEGER(KIND=JWIM) :: IFCSTEP_HOUR
      ...
      REAL(KIND=JWRU) :: DURATION, DURATION_MAX, PSTEP8
...
!     !!!! FIRST TIME AROUND !!!!
        ...
        IF (LWCOU) THEN
          ...
          PSTEP8=PSTEP
          DURATION=REAL(KSTOP,KIND=JWIB)*PSTEP8
          NDUR=INT(DURATION/DURATION_MAX)+1
          KSTOP_BY=KSTOP/NDUR
          write(0,*)'wavemdl: DURATION=',DURATION,' DURATION_MAX=',DURATION_MAX,' NDUR=',NDUR,' kstop=',kstop, ' KSTOP_BY=',KSTOP_BY
          DO ISTOP=1,KSTOP,KSTOP_BY
            IDURAT=MIN(KSTOP_BY,KSTOP-ISTOP+1)*NINT(PSTEP,JWIB)
           CALL INCDATE (CDATEE,IDURAT)
           write(0,*)"wavemdl: ISTOP=",ISTOP," IDURAT=",IDURAT," CDATEE=",CDATEE
          ENDDO

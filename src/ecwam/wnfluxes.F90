...
!     THE INTEGRATION UP TO FR(NFRE)
        DO M=1,NFRE
          ...
          YSTRESSICE(IJ) = YSTRESSICE(IJ) + SUMYICE(IJ)*CINV(IJ,M)*RHOWG_DFIM(M)
          ENDDO
        ENDIF
...
!           TAUICX(IJ) = MIN(MAX(XSTRESSICE(IJ),TAUICMIN),TAUICMAX)
!           TAUICY(IJ) = MIN(MAX(YSTRESSICE(IJ),TAUICMIN),TAUICMAX)
        ENDDO

...
SUBROUTINE GETODBRALT(IREAD, NIJALTP1, NOBS, NSATOBS)
    ...
    IF( .NOT. ASSOCIATED(GETODBRALT_HANDLER) ) THEN
        CALL WAM_ABORT("GETODBRALT_HANDLER is not associated. Make sure WAM_SETUP_ODB4WAM was called.", &
            & __FILENAME__, __LINE__)
    ELSE
...
SUBROUTINE RFL2ODB(NOBS, CLDATES, LLCLOSE)
    ...
    IF( .NOT. ASSOCIATED(RFL2ODB_HANDLER) ) THEN
        CALL WAM_ABORT("RFL2ODB_HANDLER is not associated. Make sure WAM_SETUP_ODB4WAM was called.", &
            & __FILENAME__, __LINE__)
    ELSE
...
SUBROUTINE WAM2ODB(IJS, IJL, HSOIB, HSAN, U10FG, U10AN)
    ...
    IF( .NOT. ASSOCIATED(WAM2ODB_HANDLER) ) THEN
        CALL WAM_ABORT("WAM2ODB_HANDLER is not associated. Make sure WAM_SETUP_ODB4WAM was called.", &
            & __FILENAME__, __LINE__)
    ELSE
...
SUBROUTINE WAM_ODB_CLOSE
    ...
    IF( .NOT. ASSOCIATED(WAM_ODB_CLOSE_HANDLER) ) THEN
        CALL WAM_ABORT("WAM_ODB_CLOSE_HANDLER is not associated. Make sure WAM_SETUP_ODB4WAM was called.", &
            & __FILENAME__, __LINE__)
    ELSE

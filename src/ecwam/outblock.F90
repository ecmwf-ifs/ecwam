...
! ----------------------------------------------------------------------
     ...
     &            NIPRMOUT, ITOBOUT
      ...
      USE YOWPCONS , ONLY : NPMAX !.... needed for Loki
      USE YOWTABL  , ONLY : NANGH, NFREH, NMAX !.... needed for Loki
      USE YOWPCONS , ONLY : EPSMIN !... needed for Loki
      USE YOWFRED  , ONLY : FR5, FLOGSPRDM1 !... needed for Loki
...
!!    alternative ways to determine wave height extremes 
      IF (IPFGTBL(68) /= 0 .OR. IPFGTBL(69) /= 0  .OR. &
&         IPFGTBL(68) /= 0 .OR. IPFGTBL(71) /= 0 ) THEN

...
#include "ibrmemout.intfb.h"
      ...
      INTEGER(KIND=JWIM), DIMENSION(KIJL), INTENT(IN) :: IBRMEM
      ...
      REAL(KIND=JWRO), DIMENSION(KIJL), INTENT(IN) :: NEMOSST, NEMOCICOVER, NEMOCITHICK, NEMOUCUR, NEMOVCUR
...
!       CONVERT DIRECTIONS TO DEGREES AND METEOROLOGICAL CONVENTION
      ...
      DO IH=1,NTEWH
        IF (IPFGTBL(55 + 3*NTRAIN + IH) /= 0) THEN
          TEWHMIN = REAL(IPRMINFO(55 + 3*NTRAIN + IH,4),JWRB)
          TEWHMAX = REAL(IPRMINFO(55 + 3*NTRAIN + IH,5),JWRB) 
          CALL SEBTMEAN (KIJS, KIJL, FL2ND, TEWHMIN, TEWHMAX, BOUT(:,ITOBOUT(55 + 3*NTRAIN + IH)))
!         SIGNIFICANT WAVE HEIGHT CONVERSION
...
!        BOUT(KIJS:KIJL,ITOBOUT(65 + 3*NTRAIN + NTEWH))=HMAX_N(KIJS:KIJL)
        BOUT(KIJS:KIJL,ITOBOUT(65 + 3*NTRAIN + NTEWH))=MAX(-TAUICX(KIJS:KIJL),0.0_JWRB)
      ENDIF

      IF (IPFGTBL(66 + 3*NTRAIN + NTEWH) /= 0) THEN
!        BOUT(KIJS:KIJL,ITOBOUT(66 + 3*NTRAIN + NTEWH))=CMAX_ST(KIJS:KIJL)
        BOUT(KIJS:KIJL,ITOBOUT(66 + 3*NTRAIN + NTEWH))=MAX(-TAUICY(KIJS:KIJL),0.0_JWRB)
      ENDIF

...
#include "ibrmemout.intfb.h"
      ...
      REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(IN) :: UFRIC, UPROXY, TAUW, Z0M, Z0B, CHRNCK, CITHICK
...
&         IPFGTBL(65 + 3*NTRAIN + NTEWH) /= 0 .OR. IPFGTBL(66 + 3*NTRAIN + NTEWH) /= 0 ) THEN
     ...
     &               CMAX_F, HMAX_N, CMAX_ST, HMAX_ST, PHIST)
      ...
      IF (IPFGTBL(66 + 3*NTRAIN + NTEWH) /= 0) THEN
        ...
        BOUT(KIJS:KIJL,ITOBOUT(66 + 3*NTRAIN + NTEWH))=UPROXY(KIJS:KIJL) ! josh hack
      ENDIF

      IF (IPFGTBL(67 + 3*NTRAIN + NTEWH) /= 0) THEN
        CALL IBRMEMOUT (KIJS, KIJL, IBRMEM, CICOVER, BOUT(:,ITOBOUT(67 + 3*NTRAIN + NTEWH)))
      ENDIF

      IF (IPFGTBL(68 + 3*NTRAIN + NTEWH) /= 0) THEN
        BOUT(KIJS:KIJL,ITOBOUT(68 + 3*NTRAIN + NTEWH))=TAUICX(KIJS:KIJL)
      ENDIF

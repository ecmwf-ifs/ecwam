...
SUBROUTINE GETGRBOBSTRCT(BLK2GLO, BLK2LOC, NPR, KFILE_HANDLE, KGRIB_HANDLE)
...
INTEGER(KIND=JWIM), INTENT(IN) :: KFILE_HANDLE  !! GRIB FILE HANDLE CONNECTED TO SUB GRIB BATHY INPUT
INTEGER(KIND=JWIM), INTENT(IN) :: KGRIB_HANDLE  !! GRIB HANDLE CONNECTED TO SUB GRIB BATHY INPUT

INTEGER(KIND=JWIM) :: IJ, M, K, IP, IC, IS, IX, IY
INTEGER(KIND=JWIM) :: IPARAM, KZLEV, IANGNB, IFRENB, IOBSRT
...
IF ( LSUBGRID ) THEN
  ...
  CALL FIELDG%ALLOC(NXFFS_LOC, NXFFE_LOC, NYFFS_LOC, NYFFE_LOC)
 ...
 &         XDELLA_SUB /= XDELLA .OR. XDELLO_SUB /= XDELLO ) THEN
  ...
  WRITE(IU06,*) "  GETGRBOBSTRCT : ", IRANK, NXFFS_LOC, NXFFE_LOC, NYFFS_LOC, NYFFE_LOC
  CALL FLUSH(IU06)
 ...
 &                  IUGRB=KFILE_HANDLE, NPR=NPR, KANGNB=IANGNB, KFRENB=IFRENB)
      ...
      IF ( IANGNB /= K .OR. IFRENB /= M ) THEN
        ...
        CALL FLUSH(IU06)
        CALL WAM_ABORT("Obstruction input is in the expected order !",__FILENAME__,__LINE__)
      ENDIF

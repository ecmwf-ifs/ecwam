...
SUBROUTINE GETGRBOBSTRCT(BLK2GLO, BLK2LOC, IREAD, NPR, KFILE_HANDLE, KGRIB_HANDLE, FILNM)
...
IF ( LSUBGRID ) THEN
 ...
 &                  NPR=NPR, KANGNB=IANGNB, KFRENB=IFRENB, NFILE_HANDLE=KFILE_HANDLE)
...
!$OMP     END PARALLEL DO
  ...
  DEALLOCATE(FIELD)
  IF( KFILE_HANDLE > 0 ) CALL IGRIB_CLOSE_FILE(KFILE_HANDLE)

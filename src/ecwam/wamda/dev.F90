...
!                  MEAN FREQUENCIES OF PARTITIONINGS
      REAL(KIND=JWRB) :: PKFREW(NSPEC,MPART),PKFRES(NSPEC,MPART)
!                  PEAK FREQUENCIES OF PARTITIONINGS
      INTEGER(KIND=JWIM), DIMENSION (NSPEC,MPART) :: PARTINFOW,PARTINFOS
!                  INFORMATION FOR WINDSEA-SWELL:
!                  WINDSEA (=1), MIXED (=2), OR SWELL (=0)
      REAL(KIND=JWRB) :: XKLOW(NSPEC), XKUP(NSPEC)
!                  LOWER AND UPPER SAR WAVE NUMBER. 
      REAL(KIND=JWRB) :: XKCUTOFF(NSPEC)
...
!     ----------
      ...
      INTEGER(KIND=JWIM) :: JJPARTW, JJPARTS, JSPEC, IDSAR
!                  COUNTERS
      REAL(KIND=JWRB) :: COEF, AKA
      REAL(KIND=JWRB) ::                                                &
...
!                  MEAN WAVE NUMBERS OF SAR PARTITIONINGS.
      REAL(KIND=JWRB) :: AKAW(NSPEC,MPART), AKAS(NSPEC,MPART)
...
!     ------------------------------
      ...
      DO JSPEC = 1,NSPEC
        DO JJPARTW = 1,NPARTW(JSPEC)
          ...
          FWX(JJPARTW,JSPEC) = AKA * COS( XMEANANGW(JSPEC,JJPARTW) )
          FWY(JJPARTW,JSPEC) = AKA * SIN( XMEANANGW(JSPEC,JJPARTW) )

          AKAW(JSPEC,JJPARTW)=COEF * PKFREW(JSPEC,JJPARTW)**2
        END DO
        DO JJPARTS = 1,NPARTS(JSPEC)
          AKA=COEF * XMEANFRES(JSPEC,JJPARTS)**2
          FSX(JJPARTS,JSPEC) = AKA * COS( XMEANANGS(JSPEC,JJPARTS) )
          FSY(JJPARTS,JSPEC) = AKA * SIN( XMEANANGS(JSPEC,JJPARTS) )
...
!     -----------------------
      DO JSPEC=1,NSPEC 
        DO JJPARTS = 0,MPART
          ED (JSPEC,JJPARTS) = 0.0_JWRB
          XKD(JSPEC,JJPARTS) = 0.0_JWRB
          YKD(JSPEC,JJPARTS) = 0.0_JWRB
        END DO
      END DO

...
!     ----------
      ...
      INTEGER(KIND=JWIM) :: NGAP(ISPECS:ISPECE)
      INTEGER(KIND=JWIM) :: NSPEC, ISPECS, ISPECE, MGAP, NANG, NFRE
!                 DIMENSIONS.
      REAL(KIND=JWRB) :: SPEC(0:NSPEC,NANG,NFRE)
!                 2D SPECTRUM.
      REAL(KIND=JWRB) :: FX(NANG,NFRE), FY(NANG,NFRE)
!                 POLAR COORDINATES.
      INTEGER(KIND=JWIM) :: LANG(ISPECS:ISPECE,MGAP),                   &
...
!     ----------------
      ...
      INTEGER(KIND=JWIM) :: ISPEC, IGAP, IANG, IFRE, I12, I, J
!                 LOOP INDICES.
      INTEGER(KIND=JWIM) :: LANG0(2),HANG0(2),LFRE0,HFRE0
!                 CORNERS OF FRAMES FOR TEMPORARILY USE
      REAL(KIND=JWRB) :: X, Y
!                 FREQUENCY VALUES IN CARTHESIAN COORDINATES
      REAL(KIND=JWRB) :: Z_REAL
      REAL(KIND=JWRU) :: DL_DOUBLE
!                 USED TO CHOOSE BETWEEN SGELSS OR DGELSS (LAPACK)
      REAL(KIND=JWRB) :: A(6,6), B(6), T(6), S(6), WORK(1000), RCOND
      INTEGER(KIND=JWIM) :: IRANK, INFO
...
!     LOOP OVER GAPS.
          IF (LANG(ISPEC,IGAP).EQ.1    .AND.                            &
     &        HANG(ISPEC,IGAP).EQ.NANG .OR.                             &
          ...
          ELSE
            ...
            ELSE
              ...
              IF (LANG(ISPEC,IGAP).EQ.1) THEN
                LANG0(2) = NANG
                HANG0(2) = NANG
              ELSEIF (HANG(ISPEC,IGAP).EQ.NANG) THEN
                LANG0(2) = 1
                HANG0(2) = 1
              ELSE
                LANG0(2) = 1
...
!     ---------------------------------------------------------
          ...
          DO I12 = 1,2
            DO IANG = LANG0(I12),HANG0(I12)
              DO IFRE = LFRE0,HFRE0
                IF (SPEC(ISPEC,IANG,IFRE).NE.0.0_JWRB)THEN
                  X = FX(IANG,IFRE)
                  Y = FY(IANG,IFRE)
                  T(1) = 1.0_JWRB
                  T(2) = X
...
!      ------------------------------------------------------------
          ...
          INFO  = 0
          RCOND = 5.E-4_JWRB
#ifdef WAM_HAVE_LAPACK
          IF (KIND(Z_REAL) == KIND(DL_DOUBLE)) THEN
            CALL DGELSS(6, 6, 1, A, 6, B, 6, S, RCOND, IRANK, WORK,     &
     &                  SIZE(WORK), INFO)
          ELSE
            CALL SGELSS(6, 6, 1, A, 6, B, 6, S, RCOND, IRANK, WORK,     &
     &                  SIZE(WORK), INFO)
          ENDIF
#else
          CALL WAM_ABORT("GAPINTERPOL:DGELSS or SGELSS is a LAPACK routine, and WAM is not compiled with LAPACK",&
                        &"gapinterpol.F90",__LINE__)
#endif
              ...
              DO IANG = LANG0(I12),HANG0(I12)
                DO IFRE = LFRE0,HFRE0
                  IF (SPEC(ISPEC,IANG,IFRE).EQ.GAPVAL) THEN
                    X = FX(IANG,IFRE)
                    Y = FY(IANG,IFRE)
                    SPEC(ISPEC,IANG,IFRE) = MAX(0.0_JWRB ,              &

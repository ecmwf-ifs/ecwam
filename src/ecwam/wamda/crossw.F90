...
!     ----------
     ...
     &             MINDISH, MINDIST, MINDIS(MPART)
      INTEGER(KIND=JWIM) ::                                             &
     &             JSPEC, JPARTW, JPARTS, JSAR, IDSAR,                  &
     &             JPARWH, JPARSH, IPSH, JPARWH1
!                     COUNTERS
...
!     COMPUTE DISTANCE BETWEEN SAR AND WAM PEAKS.
        ...
        DO JPARTS=1,NPARTS(IDSAR)
         ...
         MINDIS(JPARTS)=100.0_JWRB
        END DO

        DO JPARTW = 1,NPARTW(JSPEC)
          DO JPARTS = 1,NPARTS(IDSAR)
            AKABS=(0.5_JWRB*(XKP(JSPEC,JPARTW)**2+FSX(IDSAR,JPARTS)**2+ &
     ...
     &                        JSAR)=-1
                    MINDIS(NPASC2W(JSPEC,JPARTW,JSAR))=100.0_JWRB
                  END IF
     ...
     &                  DIST(JPARWH,JPARSH).LT.MINDISH .AND.            &
     &                  JPARSH.NE.JPARTS) THEN

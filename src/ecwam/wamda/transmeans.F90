...
!     ----------
      ...
      INTEGER(KIND=JWIM) :: NSPEC, NANG, NFRE
!                     DIMENSIONS.
      REAL(KIND=JWRB) :: SPECW(0:NSPEC,NANG,NFRE)
!                     2D SPECTRUM.
      REAL(KIND=JWRB) :: MEANTE(NSPEC), MEANTANG(NSPEC), MEANTFRE(NSPEC)
!                     MEAN PARAMETERS.
...
!     ----------------
      ...
      INTEGER(KIND=JWIM) :: ISPEC, IANG, IFRE
!                     LOOP VARIABLES.
...
!     --------------------
      ...
      DO ISPEC = 1,NSPEC
        SUMX0(ISPEC) = 0.0_JWRb
        SUMY0(ISPEC) = 0.0_JWRB
      END DO
      ...
      DO IANG = 1,NANG
        ...
        DO IFRE=1,NFRE
          DO ISPEC=1,NSPEC
            SUM0(ISPEC) = SUM0(ISPEC) + SPECW(ISPEC,IANG,IFRE) * DFIM(IFRE)
          END DO
        END DO
        DO ISPEC = 1,NSPEC
      ...
      DO ISPEC = 1,NSPEC
        IF( SUMY0(ISPEC).NE.0.0_JWRB .OR. SUMX0(ISPEC).NE.0.0_JWRB )THEN
          MEANTANG(ISPEC) = ATAN2(SUMY0(ISPEC),SUMX0(ISPEC))
        ELSE
          MEANTANG(ISPEC) = 0.0_JWRB
        END IF
      END DO
...
!     -------------------------
      ...
      DO IFRE = 1,NFRE
        ...
        END DO
        DO ISPEC = 1,NSPEC
          MEANTFRE(ISPEC) = MEANTFRE(ISPEC) + DFIMOFR(IFRE)*SUM0(ISPEC)
        END DO
      END DO
      ...
      DO ISPEC = 1,NSPEC
        ...
        IF( MEANTFRE(ISPEC).NE.0.0_JWRB )THEN
          MEANTFRE(ISPEC) = MEANTE(ISPEC) / MEANTFRE(ISPEC)
        ELSE
          MEANTFRE(ISPEC) = 0.0_JWRB
        END IF
      END DO

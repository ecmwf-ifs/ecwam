...
! ----------------------------------------------------------------------
      ...
      ZMX=A(IMX,IMX)*REAL(N*N,JWRB)

!**** 1. ROOT FREE CHOLESKY DECOMPOSITION  A =L D L(TRANSPOSE)
      J=1
      IF (A(1,1) <= EPS) GOTO 91
      V(1)=1.0_JWRB/A(1,1)
      IF (N == 1) GOTO 20
      X=A(2,1)*V(1)
      A(2,2)=A(2,2)-X*A(2,1)
      A(2,1)=X
      IF (A(2,2) <= EPS) GOTO 91
      V(2)=1.0_JWRB/A(2,2)
      IF (N == 2) GOTO 20
...
50    CONTINUE
      IMX=ISAMAX_WAM(N,A(1,1),NDIA)
      COND=1.0_JWRB/ABS(A(IMX,IMX)*ZMX)
...
! ----------------------------------------------------------------------
      INTEGER(KIND=JWIM) FUNCTION ISAMAX_WAM(N,A,M)
...
!     FIND THE LARGEST ABSOLUTE ELEMENT OF A , SPACED M WORDS APART
      ...
      INTEGER(KIND=JWIM), INTENT(IN) :: N, M
      REAL(KIND=JWRB), INTENT(IN) :: A(*)

      INTEGER(KIND=JWIM) :: LARGE, I, INDEX

      LARGE=1
      IF (N <= 1) GOTO 9
      INDEX=1+M
      DO I=2,N
        IF (ABS(A(INDEX)) >= ABS(A(LARGE))) LARGE=I
        INDEX=INDEX+M
      ENDDO
9     CONTINUE
      ISAMAX_WAM=LARGE

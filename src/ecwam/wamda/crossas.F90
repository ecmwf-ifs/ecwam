...
!     ----------
      ...
      INTEGER(KIND=JWIM):: ISPEC, IPARTW, IPARTS
!                        LOOP INDEXES.
      INTEGER(KIND=JWIM):: NPARTWMAX, NPARTSMAX
!                        MAXIMUM VALUE OF NPARTW/NPARTS OVER ALL 
!                        SPECTRA
      REAL(KIND=JWRB), ALLOCATABLE :: DIST(:,:,:)
!                        THE SQUARE OF THE DISTANCE BETWEEN TWO SYSTEMS
!                        IN K-SPACE.
      REAL(KIND=JWRB):: MINDIST
!                        MINIMAL DISTANCE FOR CROSS ASSIGNMENT.
      REAL(KIND=JWRB), ALLOCATABLE, DIMENSION(:,:) :: FWX, FWY
!                        MEAN WAVE NUMBERS OF WAM PARTITIONINGS.
      REAL(KIND=JWRB), ALLOCATABLE, DIMENSION(:,:) :: FSX, FSY
!                        MEAN WAVE NUMBERS OF SAR PARTITIONINGS.
      REAL(KIND=JWRB)::  AKABS
      INTEGER(KIND=JWIM):: IPARWH,IPARWH1,IPARSH,IPSH
      REAL(KIND=JWRB)::  MINDIS(MPART),MINDISH
...
!      COMPUTE WAVE NUMBER ARRAYS.
      ...
      DO ISPEC = 1,NSPEC
        DO IPARTW=1,NPARTWMAX
          FWX(ISPEC,IPARTW)=0.1E-10_JWRB
          FWY(ISPEC,IPARTW)=0.1E-10_JWRB
        END DO
      END DO
      DO ISPEC = 1,NSPEC
        DO IPARTS=1,NPARTSMAX
          FSX(ISPEC,IPARTs)=0.1E-10_JWRB
          FSY(ISPEC,IPARTs)=0.1E-10_JWRB
        END DO
      END DO
      DO ISPEC = 1,NSPEC
              ...
              IF(DIST(ISPEC,IPARTW,IPARTS).LT.MINDIST) THEN
                IF( FLAG (IPARTS).LT.0 ) THEN
                  IF(CORRELTAB(ISPEC,IPARTW).GT.0) THEN
                    ...
                    MINDIS(CORRELTAB(ISPEC,IPARTW))=100.0_JWRB
                  END IF
                  MINDIST=DIST(ISPEC,IPARTW,IPARTS)
                  CORRELTAB(ISPEC,IPARTW) = IPARTS
                  FLAG(IPARTS)=IPARTW
                  MINDIS(IPARTS)=MINDIST
                ELSEIF(DIST(ISPEC,IPARTW,IPARTS).LT.MINDIS(IPARTS)) THEN
                  MINDIST=DIST(ISPEC,IPARTW,IPARTS)
                  CORRELTAB(ISPEC,IPARTW) = IPARTS
...
!                 CORRELTAB(ISPEC,IPARTW) = 0
                END IF
              END IF
            ELSEIF(MINDIST.EQ.100.0_JWRB) THEN
                CORRELTAB(ISPEC,IPARTW) = 0
            END IF
          END DO
        END DO

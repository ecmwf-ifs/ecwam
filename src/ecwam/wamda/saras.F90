SUBROUTINE SARAS(BLK2GLO, FF_NOW, FL1)
...
#include "tustreas.intfb.h"
      ...
      INTEGER(KIND=JWIM) :: IJSG, IJLG, M, K

      INTEGER(KIND=JWIM) :: IU11,IU12,IU13, IU14
      INTEGER(KIND=JWIM), ALLOCATABLE :: PART(:,:,:,:)
...
!                   AND FREQUENCY IFRE BELONGS TO PARTITION NUMBER P.
      INTEGER(KIND=JWIM), ALLOCATABLE :: NPART(:,:) ! NUMBER OF PARTITIONINGS
      INTEGER(KIND=JWIM), ALLOCATABLE :: PARTINFO(:,:,:)
!                   INFORMATION FOR WINDSEA-SWELL:
!                   WINDSEA (=1), MIXED (=2), OR SWELL (=0)
!                   (THERE IS NO MORE THAN ONE WINDSEA PARTIONING)
      INTEGER(KIND=JWIM), ALLOCATABLE :: CORRELTAB(:,:)
...
!
      REAL(KIND=JWRB), ALLOCATABLE, DIMENSION(:,:,:) :: MEANE, MEANANG, &
     &                                       MEANFRE, PKFRE 
...
!                   DENOTES WAM IF EQUAL TO 1 AND SAR IF EQUAL TO 2.
      ...
      REAL(KIND=JPHOOK) :: ZHOOK_HANDLE ! Dr HOOK

      REAL(KIND=JWRB) :: TH0 ! FIRST DIRECTION IN WAM SPECTRUM.
      REAL(KIND=JWRB) :: FRE1 !LOWEST FREQUENCY.
      REAL(KIND=JWRB) :: ALEVEL,DISMIN
!                   FACTOR FOR THREHHOLD CHECK, AND MINIMUM
!                   DISTANCE BETWEEN TWO WAVE SYSTEMS IN K-SPACE
!                   FOR COMBINING SAR PEAKS.
      INTEGER(KIND=JWIM) :: ISPEC,ISPA ! LOOP INDICES.

      LOGICAL :: LOTEST, LOTIME, LCONTROL
!     FLAGS TO OUTPUT ON UNIT 6

      REAL(KIND=JWRB), ALLOCATABLE, DIMENSION(:,:) :: EA, XFREA, XANGA 
!         ANALYSED MEAN PARAMETERS.

      REAL(KIND=JWRB), ALLOCATABLE, DIMENSION(:) :: MEANTE, MEANTEU,    &
     &                       MEANTANG, MEANTANGU, MEANTFRE, MEANTFREU
!                   MEAN ENERGY, MEAN DIRECTION, MEAN FREQUENCY
!                   OF TOTAL SPECTRUM BEFORE AND AFTER ASSIMILATION.

      INTEGER(KIND=JWIM), ALLOCATABLE, DIMENSION(:) :: NPARTW, NINTW
      INTEGER(KIND=JWIM), ALLOCATABLE, DIMENSION(:,:) :: NIPARTINFW,    &
     &                                     CORRELTABW
      INTEGER(KIND=JWIM), ALLOCATABLE, DIMENSION(:,:,:) :: NIPARTW

      REAL(KIND=JWRB), ALLOCATABLE, DIMENSION(:,:) :: XMEANEW,          &
     &                 XMEANANGW, XMEANFREW, PKFREW, EW, XKW, YKW 
!         CHARACTERISTIC PARAMETERS (ENERGY,WAVE NUMBERS) OF WAM
!         FIRST GUESS SPECTRA.

      REAL(KIND=JWRB), ALLOCATABLE, DIMENSION(:,:) :: ED, XKD, YKD
...
!
      INTEGER(KIND=JWIM) :: IREAD  ! PE NUMBER FROM WHICH INPUT FILE WILL BE READ
      LOGICAL :: LNODATA
!
!     USED BY CROSSW
      INTEGER(KIND=JWIM), ALLOCATABLE, DIMENSION(:,:,:) :: NPASC2W,     &
     &                                                     CORRELTAS
...
IF (LHOOK) CALL DR_HOOK('SARAS',0,ZHOOK_HANDLE)
      ...
      IF (XDELLA < 0.3_JWRB) THEN
        SARCORDIA = 200000.0_JWRB
      ELSE
        SARCORDIA = 300000.0_JWRB
      ENDIF
...
!     AND GET WAM SPECTRA AT OBSERVATION LOCATION
      ...
      IF (LCONTROL) WRITE(IU06,*)'-----------REARRNGSAR BEGINS------------'

      CALL REARRNGSAR(TH0, BLK2GLO, FF_NOW, FL1)
...
!     ---------------------------------------------------------
      ...
      IF (LCONTROL) WRITE(IU06,*)'-------SELECT WAM SPECTRA FOR THE ANALYSIS --' 

      ALLOCATE(NW1D(IJSG:IJLG))

      CALL GETWSPEC(IJSG, IJLG, BLK2GLO, FF_NOW, FL1, NW1D)
...
!     ------------------------------------------------------------
     ...
     &            THWW, SPECW , NW1D, NIPARTW , CORRELTABW ,            &
     &            XMEANEW, XMEANANGW, XMEANFREW,                        &
     &            EA , XANGA , XFREA,                                   &
...
4000  CONTINUE
      CALL MERGESARCOR(IJSG, IJLG, FL1, NW1D)

      IF ( KIJL4CHNK(NCHNK) < NPROMA_WAM ) THEN
        ! update fictitious points
         DO M = 1, NFRE
           DO K = 1, NANG
             FL1(KIJL4CHNK(NCHNK)+1:NPROMA_WAM,K,M,NCHNK) =  FL1(1,K,M,NCHNK)
           ENDDO
         ENDDO
      ENDIF

...
!---------------------------------------------------------------------
      ...
      DO ISPEC = 1,NSPEC
        IPART = 1
        DO WHILE( IPART.LE.NPART(ISPEC)-1 .AND. PARTINFO(ISPEC,IPART).EQ.0)
          JPART = IPART + 1
     ...
     &          SPEC(ISPEC,PEAKANG(ISPEC,JPART),PEAKFRE(ISPEC,JPART)))
            ...
            DO WHILE( COMBINE .AND. IDIST.LT.DIST )
              IF( SUMANG.GT.SUMFRE )THEN
                ...
                IDIST = IDIST + 1
                IF( SPEC(ISPEC,IANG,IFRE).LT.LEVEL*PEAKENERGY ) COMBINE = .FALSE.

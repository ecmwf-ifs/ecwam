...
! ----------------------------------------------------------------------
      ...
      REAL(KIND=JWRB), ALLOCATABLE :: R4DUM(:,:,:,:)
      REAL(KIND=JWRB), ALLOCATABLE :: R2DUM(:,:)
      REAL(KIND=JWRB), ALLOCATABLE :: R1DUM(:)

      INTEGER(KIND=JWIM) :: KSPEC
!                FUTURE DIMENSION OF RESIZABLE ARRAYS  
      INTEGER(KIND=JWIM) :: I,ISPEC, IANG, IFRE, NEWNSPEC
...
!     DO THE SWITCH:
      ...
      ALLOCATE(R1DUM(NSPEC))

      R1DUM=XKLOW
      DEALLOCATE(XKLOW)
      ALLOCATE(XKLOW(NEWNSPEC))
      DO ISPEC=1,MIN(NSPEC,NEWNSPEC)
        XKLOW(ISPEC)=R1DUM(ISPEC)
      ENDDO

      R1DUM=XKUP
      DEALLOCATE(XKUP)
      ALLOCATE(XKUP(NEWNSPEC))
      DO ISPEC=1,MIN(NSPEC,NEWNSPEC)
        XKUP(ISPEC)=R1DUM(ISPEC)
      ENDDO

      R1DUM=XKCUTOFF
      DEALLOCATE(XKCUTOFF)
      ALLOCATE(XKCUTOFF(NEWNSPEC))
      DO ISPEC=1,MIN(NSPEC,NEWNSPEC)
        XKCUTOFF(ISPEC)=R1DUM(ISPEC)
      ENDDO

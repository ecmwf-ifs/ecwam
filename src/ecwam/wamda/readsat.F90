...
#include "confile.intfb.h"
      ...
      INTEGER(KIND=JWIM),INTENT(IN) :: IU06, IUME
      INTEGER(KIND=JWIM),INTENT(INOUT) :: IDENTI, ISENSOR, IDES_HS, IDES_WS

      REAL(KIND=JWRB), INTENT(INOUT) :: RLAT, RLON, SWH, WS
      ...
      INTEGER(KIND=JWIM) :: IFAIL, IRS
      REAL(KIND=JPHOOK) :: ZHOOK_HANDLE
      CHARACTER(LEN=12) :: CTIME
      CHARACTER(LEN=14), SAVE :: CDTGE1
...
! ----------------------------------------------------------------------
      IF (LHOOK) CALL DR_HOOK('READSAT',0,ZHOOK_HANDLE)

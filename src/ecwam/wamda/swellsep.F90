...
!     ----------
      ...
      IMPLICIT NONE
#include "combinepeaks.intfb.h"
#include "means.intfb.h"
#include "partitioning.intfb.h"

      INTEGER(KIND=JWIM) :: NSPEC  ! NUMBER OF SPECTRA TO BE PARTITIONED
      INTEGER(KIND=JWIM) :: MPART  ! MAXIMUM NUMBER OF PARTITIONING.
      INTEGER(KIND=JWIM) :: NPART(NSPEC)
!                      NUMBER OF PARTITIONINGS,
!                      WHERE NPART(ISPEC) IS THE NUMBER OF 
!                      PARTITIONINGS OF SPECTRUM ISPEC.
      INTEGER(KIND=JWIM) :: NANG   ! NUMBER OF SPECTRAL DIRECTIONS
      INTEGER(KIND=JWIM) :: NFRE   ! NUMBER OF SPECTRAL FREQUENCIES
      REAL(KIND=JWRB) :: SPEC(0:NSPEC,NANG,NFRE) ! SPECTRA
...
!                      FREQUENCY IFRE
      INTEGER(KIND=JWIM) :: PARTINFO(NSPEC,MPART)
...
!                      PARTITIONING IPART
      REAL(KIND=JWRB) :: MEANANG(NSPEC,MPART)
!                      MEAN DIRECTIONS IN RADIAN, LIKE ABOVE
      REAL(KIND=JWRB) :: MEANFRE(NSPEC,MPART)
!                      MEAN FREQUENCIES IN HZ, LIKE ABOVE
      REAL(KIND=JWRB) :: PKFRE(NSPEC,MPART)
!                      PEAK FREQUENCIES IN HZ, LIKE ABOVE
      REAL(KIND=JWRB) :: US(NSPEC)  ! USTAR
      REAL(KIND=JWRB) :: THW(NSPEC) ! WIND DIRECTION IN DEGREE
      REAL(KIND=JWRB) :: TH0        ! FIRST DIRECTION IN SPECTRUM.
      REAL(KIND=JWRB) :: LEVEL
...
!     ----------
      ...
      INTEGER(KIND=JWIM) :: PEAKANG( NSPEC , NANG*NFRE/2 )
!                      CONTAINS DIRECTIONAL INDICES OF PEAKS.
      INTEGER(KIND=JWIM) :: PEAKFRE( NSPEC , NANG*NFRE/2 )
!                      CONTAINS FREQUENCY INDICES OF PEAKS.
      REAL(KIND=JWRB), ALLOCATABLE, DIMENSION(:,:) :: FX, FY, SPREAD
!            MEAN WAVE NUMBERS, SQUARES OF THE SPREAD
!
      REAL(KIND=JWRB), ALLOCATABLE, DIMENSION(:,:) :: SUMX, SUMXX, SUMY, SUMYY 
!                      WORK SPACE FOR SUMMATION WHILE 
!                      COMPUTING MEANE AND SPREAD.
      REAL(KIND=JWRB), ALLOCATABLE, DIMENSION(:,:) :: SUM0, SUMX0, SUMXX0, SUMY0, SUMYY0
!                      WORK SPACE ONLY USED IN SUBROUTINE SUMENERGY.
      REAL(KIND=JWRB) :: FR(NFRE) ! FREQUENCY ARRAY.
      REAL(KIND=JWRB) :: COSTH(NANG) , SINTH(NANG) ! COSIN AND SIN TABLE

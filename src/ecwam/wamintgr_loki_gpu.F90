...
!$acc data present(FL1, WVPRPT, WVENVI, BLK2GLO)

IF (CDATE == CDTPRA) THEN
  TIME0=-WAM_USER_CLOCK()
  CALL PROPAG_WAM(BLK2GLO, WVPRPT%WAVNUM, WVPRPT%CGROUP, WVPRPT%OMOSNH2KD, FL1%PTR,& 
...
!$acc parallel loop gang vector_length(NPROMA_WAM) default(present) copyin(NPROMA_WAM)
      DO ICHNK=1,NCHNK
         CALL IMPLSCH (1, NPROMA_WAM, FL1%PTR(:,:,:,ICHNK),   &
...
#ifdef _OPENACC
!$acc kernels present(MIJ,FL1,XLLWS)
#else

...
USE YOWCOUP  , ONLY : LWNEMOCOU, NEMONTAU, LWCOU
...
CALL VARS_4D%GET_DEVICE_DATA_RDWR(VARS_4D, FL1=.TRUE.)
CALL WVPRPT%GET_DEVICE_DATA_RDONLY(WVPRPT)
CALL WVENVI%GET_DEVICE_DATA_RDONLY(WVENVI, DEPTH=.TRUE., DELLAM1=.TRUE., COSPHM1=.TRUE., UCUR=.TRUE., VCUR=.TRUE., &
&                                  EMAXDPT=.TRUE., IOBND=.TRUE., IODP=.TRUE., IBRMEM=.TRUE.)
CALL BLK2GLO%GET_DEVICE_DATA_RDONLY(BLK2GLO)
IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',1,ZHOOK_HANDLE_DATA_OFFLOAD)
...
CALL FF_NOW%GET_DEVICE_DATA_RDWR(FF_NOW, AIRD=.TRUE., WDWAVE=.TRUE., CICOVER=.TRUE., WSWAVE=.TRUE.,  &
& WSTAR=.TRUE., UFRIC=.TRUE., TAUW=.TRUE., TAUWDIR=.TRUE., Z0M=.TRUE., Z0B=.TRUE.,  &
& CHRNCK=.TRUE., CITHICK=.TRUE., USTRA=.TRUE., VSTRA=.TRUE.)
CALL FF_NEXT%GET_DEVICE_DATA_RDONLY(FF_NEXT, AIRD=.TRUE., WDWAVE=.TRUE., CICOVER=.TRUE., WSWAVE=.TRUE.,  &
& WSTAR=.TRUE., UFRIC=.TRUE., TAUW=.TRUE., TAUWDIR=.TRUE., Z0M=.TRUE., Z0B=.TRUE.,  &
& CHRNCK=.TRUE., CITHICK=.TRUE., USTRA=.TRUE., VSTRA=.TRUE.)
IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',1,ZHOOK_HANDLE_DATA_OFFLOAD)
...
! COMPUTE UPDATE DUE TO SOURCE TERMS
  ...
  IF (LLSOURCE) THEN
      ...
      IF (LWNEMOCOU .AND. (.NOT.LWCOU)) THEN
        CALL WAM2NEMO%GET_DEVICE_DATA_RDWR(WAM2NEMO, NEMOTAUICX=.TRUE., NEMOTAUICY=.TRUE., NEMOWSWAVE=.TRUE., NEMOPHIF=.TRUE., &
        & NPHIEPS=.TRUE., NTAUOC=.TRUE., NSWH=.TRUE., NMWP=.TRUE., NEMOTAUX=.TRUE., NEMOTAUY=.TRUE.)
      ELSE
        CALL WAM2NEMO%GET_DEVICE_DATA_WRONLY(WAM2NEMO, NEMOTAUICX=.TRUE., NEMOTAUICY=.TRUE., NEMOWSWAVE=.TRUE., NEMOPHIF=.TRUE., &
        & NPHIEPS=.TRUE., NTAUOC=.TRUE., NSWH=.TRUE., NMWP=.TRUE., NEMOTAUX=.TRUE., NEMOTAUY=.TRUE.)
      ENDIF
      CALL WAM2NEMO%GET_DEVICE_DATA_WRONLY(WAM2NEMO, NEMOUSTOKES=.TRUE., NEMOVSTOKES=.TRUE., NEMOSTRN=.TRUE.)
      CALL INTFLDS%GET_DEVICE_DATA_WRONLY(INTFLDS, WSEMEAN=.TRUE., WSFMEAN=.TRUE., USTOKES=.TRUE., &
      & VSTOKES=.TRUE., STRNMS=.TRUE., TAUXD=.TRUE., TAUYD=.TRUE., TAUOCXD=.TRUE., &
      & TAUOCYD=.TRUE., TAUOC=.TRUE., PHIOCD=.TRUE., PHIEPS=.TRUE., PHIAW=.TRUE., &
      & TAUICX=.TRUE., TAUICY=.TRUE.)
      CALL VARS_4D%GET_DEVICE_DATA_WRONLY(VARS_4D, XLLWS=.TRUE.)
      CALL MIJ%GET_DEVICE_DATA_WRONLY(MIJ)
      IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',1,ZHOOK_HANDLE_DATA_OFFLOAD)
...
!$loki end structured-data
      ...
      IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',0,ZHOOK_HANDLE_DATA_OFFLOAD)
      CALL VARS_4D%SYNC_HOST_RDONLY(VARS_4D, FL1=.TRUE., QUEUE=4)
      CALL FF_NOW%SYNC_HOST_RDONLY(FF_NOW, QUEUE=4)
      CALL WVENVI%SYNC_HOST_RDONLY(WVENVI, QUEUE=5)
      IF (LWNEMOCOU .AND. (.NOT.LWCOU)) CALL WAM2NEMO%SYNC_HOST_RDONLY(WAM2NEMO, QUEUE=6)
      IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',1,ZHOOK_HANDLE_DATA_OFFLOAD)

...
TYPE(WAM_TRACE) :: TRACE, TRACE_DATA_OFFLOAD, TRACE_IMPLSCH
...
! COMPUTE UPDATE DUE TO SOURCE TERMS
  ...
  IF (LLSOURCE) THEN
      ...
      CALL INTFLDS%GET_DEVICE_DATA_RDWR(TAUICX=.TRUE., TAUICY=.TRUE.)
      CALL VARS_4D%GET_DEVICE_DATA_RDWR(XLLWS=.TRUE.)
      CALL MIJ%GET_DEVICE_DATA_RDWR()
      IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',1,ZHOOK_HANDLE_DATA_OFFLOAD)
      CALL TRACE_DATA_OFFLOAD%FINAL()

      CALL TRACE_IMPLSCH%INIT(__FILENAME__,__LINE__,"IMPLSCH")

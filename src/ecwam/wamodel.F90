...
!        ----------------------------
      ...
      CALL XLLWS%SYNC_DEVICE_RDWR()
      CALL MIJ%SYNC_DEVICE_RDWR()
...
!           -------------------------
        ...
        ENDIF
        IF(IBOUNC == 1)THEN
            CALL WAM_ABORT("WAMODEL: IBOUNC==1 NOT SUPPORTED FOR GPU OFFLOAD")
        ENDIF
#endif
...
#ifdef WAM_GPU
          CALL OUTBS_LOKI_GPU (MIJ%PTR, FL1%PTR, XLLWS%PTR, &
...
!       ----------------------------------------------------
        IF (LWNEMOCOU .AND. (.NOT.LWCOU)) THEN
          ...
          IF (MOD(NEMOWSTEP,NEMOFRCO) == 0) THEN
            ...
            CALL WAM2NEMO%SYNC_HOST_RDONLY()
...
!*    BRANCHING BACK TO 1.0 FOR NEXT PROPAGATION STEP.
      ...
      CALL FF_NOW%SYNC_HOST_RDWR()
      CALL FF_NEXT%SYNC_HOST_RDWR()
      CALL WAM2NEMO%SYNC_HOST_RDWR()
      CALL INTFLDS%SYNC_HOST_RDWR()
      CALL FL1%SYNC_HOST_RDWR()
      CALL XLLWS%SYNC_HOST_RDWR()

...
#include "wamintgr.intfb.h"
      ...
      INTEGER(KIND=JWIM) :: IPLUME_DIFDATE
#endif
...
!        ----------------------------
      ...
      CALL WVPRPT_LAND%SYNC_DEVICE_RDONLY(WVPRPT_LAND, QUEUE=1)
      CALL VARS_4D%SYNC_DEVICE_RDWR(VARS_4D, FL1=.TRUE., QUEUE=1)
      CALL BLK2GLO%SYNC_DEVICE_RDONLY(BLK2GLO, QUEUE=1)
      CALL WVPRPT%SYNC_DEVICE_RDONLY(WVPRPT, QUEUE=1)
      CALL WVENVI%SYNC_DEVICE_RDONLY(WVENVI, DEPTH=.TRUE., DELLAM1=.TRUE., COSPHM1=.TRUE., UCUR=.TRUE., VCUR=.TRUE., &
      &                              EMAXDPT=.TRUE., IOBND=.TRUE., IODP=.TRUE., IBRMEM=.TRUE., QUEUE=1)
      CALL FF_NOW%SYNC_DEVICE_RDWR(FF_NOW, AIRD=.TRUE., WDWAVE=.TRUE., CICOVER=.TRUE., WSWAVE=.TRUE.,  &
      & WSTAR=.TRUE., UFRIC=.TRUE., TAUW=.TRUE., TAUWDIR=.TRUE., Z0M=.TRUE., Z0B=.TRUE.,  &
      & CHRNCK=.TRUE., CITHICK=.TRUE., USTRA=.TRUE., VSTRA=.TRUE., QUEUE=2)
      CALL FF_NEXT%SYNC_DEVICE_RDONLY(FF_NEXT, AIRD=.TRUE., WDWAVE=.TRUE., CICOVER=.TRUE., WSWAVE=.TRUE.,  &
      & WSTAR=.TRUE., UFRIC=.TRUE., TAUW=.TRUE., TAUWDIR=.TRUE., Z0M=.TRUE., Z0B=.TRUE.,  &
      & CHRNCK=.TRUE., CITHICK=.TRUE., USTRA=.TRUE., VSTRA=.TRUE., QUEUE=2)
      IF (LWNEMOCOU .AND. (.NOT.LWCOU)) THEN
         CALL WAM2NEMO%SYNC_DEVICE_RDWR(WAM2NEMO, NEMOTAUICX=.TRUE., NEMOTAUICY=.TRUE., NEMOWSWAVE=.TRUE., NEMOPHIF=.TRUE., &
         & NPHIEPS=.TRUE., NTAUOC=.TRUE., NSWH=.TRUE., NMWP=.TRUE., NEMOTAUX=.TRUE., NEMOTAUY=.TRUE., QUEUE=3)
      ENDIF
...
!       SET TIME COUNTER.
 ...
 &                       WAM2NEMO, MIJ, VARS_4D)
          ...
          CALL DIFDATE(CDATEA, CDTIMP, IPLUME_DIFDATE)
          ! Run the wave plugins
          CALL WAM_PLUGINS_RUN(PLUME_FLDS, IPLUME_DIFDATE, .TRUE.)
#endif
        ENDDO
...
!           OUTPUT POINT SPECTRA (not usually used at ECMWF)
            ...
            CALL VARS_4D%GET_HOST_DATA_RDONLY(VARS_4D, FL1=.TRUE.)
            CALL FF_NOW%GET_HOST_DATA_RDONLY(FF_NOW, AIRD=.TRUE., WDWAVE=.TRUE., CICOVER=.TRUE., WSWAVE=.TRUE.,  &
            & WSTAR=.TRUE., UFRIC=.TRUE., TAUW=.TRUE., TAUWDIR=.TRUE., Z0M=.TRUE., Z0B=.TRUE.,  &
...
!                 --------------------------------------
            IF ( .NOT.LGRIBOUT .OR. LDWRRE ) THEN
              ...
              & WSTAR=.TRUE., UFRIC=.TRUE., TAUW=.TRUE., TAUWDIR=.TRUE., Z0M=.TRUE., Z0B=.TRUE.,  &
              & CHRNCK=.TRUE., CITHICK=.TRUE., USTRA=.TRUE., VSTRA=.TRUE.)
              CALL WVENVI%GET_HOST_DATA_RDONLY(WVENVI, DEPTH=.TRUE., DELLAM1=.TRUE., COSPHM1=.TRUE., UCUR=.TRUE., VCUR=.TRUE., &
...
!       ----------------------------------------------------
        IF (LWNEMOCOU .AND. (.NOT.LWCOU)) THEN
          ...
          IF (MOD(NEMOWSTEP,NEMOFRCO) == 0) THEN
            ...
            CALL WAM2NEMO%GET_HOST_DATA_RDONLY(WAM2NEMO, NEMOUSTOKES=.TRUE., NEMOVSTOKES=.TRUE., NEMOSTRN=.TRUE.,  &
            & NPHIEPS=.TRUE., NTAUOC=.TRUE., NSWH=.TRUE., NMWP=.TRUE.)
            CALL WAM2NEMO%GET_HOST_DATA_RDWR(WAM2NEMO, NEMOTAUX=.TRUE., NEMOTAUY=.TRUE., NEMOWSWAVE=.TRUE., NEMOPHIF=.TRUE., &
            &                                NEMOTAUICX=.TRUE., NEMOTAUICY=.TRUE.)
            IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',1,ZHOOK_HANDLE_DATA_OFFLOAD)
#endif
...
#ifdef WAM_GPU
            IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',0,ZHOOK_HANDLE_DATA_OFFLOAD)
            CALL WAM2NEMO%SYNC_DEVICE_RDWR(WAM2NEMO, NEMOUSTOKES=.TRUE., NEMOVSTOKES=.TRUE., NEMOSTRN=.TRUE.,  &
...
#ifdef WAM_GPU
      IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',0,ZHOOK_HANDLE_DATA_OFFLOAD)
      CALL WVPRPT%GET_HOST_DATA_RDWR(WVPRPT)
      CALL WVENVI%GET_HOST_DATA_RDWR(WVENVI)
      CALL FF_NOW%GET_HOST_DATA_RDWR(FF_NOW)
      CALL FF_NEXT%GET_HOST_DATA_RDWR(FF_NEXT)
      IF(IASSI == 1) CALL WAM2NEMO%GET_HOST_DATA_RDWR(WAM2NEMO)
      CALL INTFLDS%GET_HOST_DATA_RDWR(INTFLDS)
      CALL VARS_4D%GET_HOST_DATA_RDWR(VARS_4D, FL1=.TRUE.)
      CALL BLK2GLO%GET_HOST_DATA_RDONLY(BLK2GLO)

      CALL WVPRPT%DELETE_DEVICE_DATA(WVPRPT)
      CALL WVENVI%DELETE_DEVICE_DATA(WVENVI)
      CALL FF_NOW%DELETE_DEVICE_DATA(FF_NOW)
      CALL FF_NEXT%DELETE_DEVICE_DATA(FF_NEXT)
      CALL WAM2NEMO%DELETE_DEVICE_DATA(WAM2NEMO)
      CALL INTFLDS%DELETE_DEVICE_DATA(INTFLDS)
      CALL VARS_4D%DELETE_DEVICE_DATA(VARS_4D)
      CALL MIJ%DELETE_DEVICE_DATA(MIJ)
      IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',1,ZHOOK_HANDLE_DATA_OFFLOAD)
#endif

...
#include "wamintgr.intfb.h"
      ...
      TYPE(WAM_TRACE) :: TRACE, TRACE_ADVECTION_LOOP, TRACE_SECTION
      TYPE(WAM_TRACE) :: TRACE_DATA_OFFLOAD
      TYPE(WAM_TRACE) :: TRACE_DBG

! ----------------------------------------------------------------------

      CALL TRACE%INIT(__FILENAME__,__LINE__,"WAMODEL")
...
!        ----------------------------
...
#ifdef WAM_GPU
      CALL TRACE_DATA_OFFLOAD%INIT(__FILENAME__,__LINE__, TITLE="UPDATE_DEVICE", LABEL="DATA_OFFLOAD")
      ...
      CALL TRACE_DATA_OFFLOAD%FINAL()
#endif

      CALL TRACE_ADVECTION_LOOP%INIT(__FILENAME__,__LINE__,"ADVECTION_LOOP")
...
!            -------------------------------------------------------
        CALL TRACE_SECTION%INIT(__FILENAME__,__LINE__,"1.2 PROPAGATE")
!       SET TIME COUNTER.
        CDATE   = CDTPRA
        CDATEWH = CDATEWO
        ILOOP = 1
...
!       1.5 COMPUTE OUTPUT PARAMETERS FIELDS AND PRINT OUT NORMS
!           ----------------------------------------------------
        IF ( (CDTINTT == CDTPRO .OR. LRST) .AND. NIPRMOUT > 0 ) THEN
          CALL TRACE_SECTION%INIT(__FILENAME__,__LINE__,"1.5 FIELD/NORM OUTPUT")
     ...
     &                BOUT)
          CALL TRACE_DATA_OFFLOAD%INIT(__FILENAME__,__LINE__, TITLE="UPDATE_HOST", LABEL="DATA_OFFLOAD")
          IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',0,ZHOOK_HANDLE_DATA_OFFLOAD)
...
!*      1.6 POINT OUTPUT (not usually used at ECMWF)
!           ----------------------------------------
        IF ( NGOUT > 0 .AND. (CDTINTT == CDTPRO .OR. LRST) ) THEN
            CALL TRACE_SECTION%INIT(__FILENAME__,__LINE__,"1.6 POINT OUTPUT")
...
!         OTHERWISE THE OUTPUT WILL OCCUR IN WAMASSI.
        ...
        IF (LSV .OR. LRST) THEN
          ...
          CALL TRACE_SECTION%INIT(__FILENAME__,__LINE__,"1.8 SAVE FIELDS")
...
!           -------------------------------------------------
        IF (LWAMANOUT .AND. CDTINTT == CDTPRO .AND. NIPRMOUT > 0 ) THEN
          CALL TRACE_SECTION%INIT(__FILENAME__,__LINE__,"1.9 WRITE INTGR DATA TO FDB/FILE")
!         IF THE OUTPUT TIME IS NOT AN ANALYSIS TIME THEN TYPE FG or 4V
...
#ifdef WAM_GPU
        ! This update is always called regardless if following is needed?
        CALL TRACE_DBG%INIT(__FILENAME__,__LINE__, TITLE="UPDATE_DEVICE", LABEL="DATA_OFFLOAD")
        IF (LHOOK) CALL DR_HOOK('DATA_OFFLOAD',0,ZHOOK_HANDLE_DATA_OFFLOAD)
        CALL VARS_4D%SYNC_DEVICE_RDWR(FL1=.TRUE., QUEUE=1)
        CALL WVENVI%SYNC_DEVICE_RDWR(DEPTH=.TRUE., DELLAM1=.TRUE., COSPHM1=.TRUE., UCUR=.TRUE., VCUR=.TRUE., &
        &                            EMAXDPT=.TRUE., IOBND=.TRUE., IODP=.TRUE., QUEUE=1)
...
!*    BRANCHING BACK TO 1.0 FOR NEXT PROPAGATION STEP.
      ...
      CALL TRACE_ADVECTION_LOOP%FINAL()
      ...
      CALL TRACE_DATA_OFFLOAD%INIT(__FILENAME__,__LINE__, TITLE="DEALLOCATE_DEVICE", LABEL="DATA_OFFLOAD")

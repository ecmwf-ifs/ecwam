...
REAL(KIND=JWRB), DIMENSION(KIJL) :: RLX, RLY, AXT, AYT, AXY
REAL(KIND=JWRB), DIMENSION(KIJL) :: WMDX, WMDY ! SPACE TIME EXTREME OVER WMDX x WMDY m**2 AREA
REAL(KIND=JWRB), DIMENSION(KIJL) :: WMDUR ! TIME EXTREME OVER WMDUR sec.
...
WHERE (EMEAN(KIJS:KIJL) > ZEPSILON)
   AXY(KIJS:KIJL) = AXY(KIJS:KIJL)/SQRT(RLX(KIJS:KIJL)*RLY(KIJS:KIJL))
   AXT(KIJS:KIJL) = AXT(KIJS:KIJL)/(ZPI*SQRT(RLX(KIJS:KIJL)*T2(KIJS:KIJL)))
   AYT(KIJS:KIJL) = AYT(KIJS:KIJL)/(ZPI*SQRT(RLY(KIJS:KIJL)*T2(KIJS:KIJL)))
   ...
   WMDUR(KIJS:KIJL) = XNWVP*T2(KIJS:KIJL)
ELSEWHERE
   AXY(KIJS:KIJL) = 0._JWRB
   AXT(KIJS:KIJL) = 0._JWRB
   AYT(KIJS:KIJL) = 0._JWRB

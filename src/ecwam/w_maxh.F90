...
! ADD HIGH FREQUENCY TAIL CONTRIBUTION TO HS
DELT25 = WETAIL*FR(NFRE)*DELTH
DO IJ = KIJS, KIJL
  HS(IJ) = HS(IJ) + DELT25*FF(IJ,NFRE)
ENDDO
...
DO IJ = KIJS, KIJL
  ...
  ACFS(1) = SUM( COS( OMEGA(:)*TLGS(1) ) * FF(IJ,:) )
  ACFS(4) = SUM( COS( OMEGA(:)*TLGS(4) ) * FF(IJ,:) )
  ACFS(2) = SUM( COS( OMEGA(:)*TLGS(2) ) * FF(IJ,:) )
  ACFS(3) = SUM( COS( OMEGA(:)*TLGS(3) ) * FF(IJ,:) )

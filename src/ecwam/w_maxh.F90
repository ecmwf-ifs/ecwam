...
USE YOWPCONS , ONLY : G        ,ZPI      ,GAMMA_E
...
REAL(KIND=JWRB), DIMENSION(KIJL,NANG,NFRE), INTENT(IN) :: F  !! BLOCK OF SPECTRA
REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(IN)    :: DEPTH     !! DEPTH
REAL(KIND=JWRB), DIMENSION(KIJL,NFRE), INTENT(IN) :: WAVNUM  !! WAVE NUMBER
REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(OUT)   :: CMAX_F    !! MAXIMUM CREST H.- TIME (FORRISTALL)
REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(OUT)   :: HMAX_N    !! MAXIMUM WAVE H.- TIME (NAESS)
REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(OUT)   :: CMAX_ST   !! MAXIMUM CREST H.- SPACE-TIME (STQD)
REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(OUT)   :: HMAX_ST   !! MAXIMUM WAVE H.- SPACE-TIME (STQD)
REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(OUT)   :: PHIST     !! 1st minimum of the aotocovariance function
...
REAL(KIND=JWRB), PARAMETER :: XNWVP = 100._JWRB  !! NUMBER OF WAVE PERIODS TO SET WMDUR_ST (if constant value is not used)
REAL(KIND=JWRB), PARAMETER :: TOL = 0.01_JWRB ! GOLDEN SEARCH TOLERANCE

INTEGER(KIND=JWIM) :: IJ, IT, M, K
INTEGER(KIND=JWIM), DIMENSION(KIJL) :: K_THMAX
...
REAL(KIND=JWRB), DIMENSION(KIJL) :: HS, RLX, RLY, AXT, AYT, AXY
REAL(KIND=JWRB), DIMENSION(KIJL) :: WMDX, WMDY ! SPACE TIME EXTREME OVER WMDX x WMDY m**2 AREA
REAL(KIND=JWRB), DIMENSION(KIJL) :: WMDUR_TD ! TIME EXTREME OVER WMDUR_TD for TIME DOMAIN in sec.
REAL(KIND=JWRB), DIMENSION(KIJL) :: WMDUR_ST ! TIME EXTREME OVER WMDUR_ST for SPACE TIME in sec.
REAL(KIND=JWRB), DIMENSION(KIJL) :: FMAX
REAL(KIND=JWRB), DIMENSION(KIJL) :: ACF, T1, T2, EMEAN 
REAL(KIND=JWRB), DIMENSION(KIJL) :: TEMP, TEMP_X, TEMP_Y, TEMP_X2, TEMP_Y2, TEMP_XY
REAL(KIND=JWRB), DIMENSION(KIJL) :: RNI, RMU 
REAL(KIND=JWRB), DIMENSION(NFRE) :: OMEGA
REAL(KIND=JWRB), DIMENSION(KIJL, NANG) :: CX, CY, CX2, CY2, CXCY
REAL(KIND=JWRB), DIMENSION(KIJL, NFRE) :: TEMPDFIM
...
ACF(KIJS:KIJL) = 0._JWRB
T1(KIJS:KIJL) = 0._JWRB
T2(KIJS:KIJL) = 0._JWRB
RLX(KIJS:KIJL) = 0._JWRB
RLY(KIJS:KIJL) = 0._JWRB
AXT(KIJS:KIJL) = 0._JWRB
AXY(KIJS:KIJL) = 0._JWRB
AYT(KIJS:KIJL) = 0._JWRB
EMEAN(KIJS:KIJL) = 0._JWRB
...
DO M = 1,NFRE
   TEMP(KIJS:KIJL) = 0._JWRB
   TEMP_X(KIJS:KIJL) = 0._JWRB
   TEMP_Y(KIJS:KIJL) = 0._JWRB
   TEMP_X2(KIJS:KIJL) = ZEPSILON 
   TEMP_Y2(KIJS:KIJL) = ZEPSILON 
   TEMP_XY(KIJS:KIJL) = 0._JWRB
   ...
   DO IJ = KIJS, KIJL
     ...
     XK2_DFIM = XK2*DFIM(M)
     RLX(IJ) = RLX(IJ) + TEMP_X2(IJ)*XK2_DFIM
     RLY(IJ) = RLY(IJ) + TEMP_Y2(IJ)*XK2_DFIM
     AXY(IJ) = AXY(IJ) + TEMP_XY(IJ)*XK2_DFIM
     XK_ZPI_DFIM = XK*ZPI*DFIMFR(M)
     AXT(IJ) = AXT(IJ) + TEMP_X(IJ)*XK_ZPI_DFIM
     AYT(IJ) = AYT(IJ) + TEMP_Y(IJ)*XK_ZPI_DFIM
   ENDDO
ENDDO

! ADD HIGH FREQUENCY TAIL CONTRIBUTION AND COMPUTE HS
DELT25 = WETAIL*FR(NFRE)*DELTH
DO IJ = KIJS, KIJL
  IF ( EMEAN(IJ) > ZEPSILON ) THEN
   EM = EMEAN(IJ) + DELT25*TEMP(IJ)
   HS(IJ) = 4.0_JWRB*SQRT(EM)
  ELSE
    HS(IJ) = 0.0_JWRB
  ENDIF
ENDDO

WHERE (EMEAN(KIJS:KIJL) > ZEPSILON)
   AXY(KIJS:KIJL) = MIN(AXY(KIJS:KIJL)/SQRT(RLX(KIJS:KIJL)*RLY(KIJS:KIJL)), 1._JWRB)
   AXT(KIJS:KIJL) = MIN(AXT(KIJS:KIJL)/(ZPI*SQRT(RLX(KIJS:KIJL)*T2(KIJS:KIJL))), 1._JWRB)
   AYT(KIJS:KIJL) = MIN(AYT(KIJS:KIJL)/(ZPI*SQRT(RLY(KIJS:KIJL)*T2(KIJS:KIJL))), 1._JWRB)
   RLX(KIJS:KIJL) = ZPI*SQRT(EMEAN(KIJS:KIJL)/RLX(KIJS:KIJL))
   RLY(KIJS:KIJL) = ZPI*SQRT(EMEAN(KIJS:KIJL)/RLY(KIJS:KIJL))
   RNI(KIJS:KIJL) = SQRT(MAX(EMEAN(KIJS:KIJL)*T2(KIJS:KIJL)/T1(KIJS:KIJL)**2 - 1._JWRB,ZEPSILON))
   RMU(KIJS:KIJL) = (ZPI*T1(KIJS:KIJL))**2*(1._JWRB-RNI+RNI**2)/(G*EMEAN(KIJS:KIJL)**THREEHALF)
   T1(KIJS:KIJL) = MIN(MAX(EMEAN(KIJS:KIJL)/T1(KIJS:KIJL),TMIN),TMAX)
   T2(KIJS:KIJL) = MIN(MAX(SQRT(EMEAN(KIJS:KIJL)/T2(KIJS:KIJL)),TMIN),TMAX)
!!!   WMDUR_TD = XNWVP*T2
   WMDX(KIJS:KIJL) = MAX(RLX(KIJS:KIJL),WVLMIN)
   WMDY(KIJS:KIJL) = MAX(RLY(KIJS:KIJL),WVLMIN)
   WMDUR_ST(KIJS:KIJL) = XNWVP*T2(KIJS:KIJL)
ELSEWHERE
   AXY(KIJS:KIJL) = 0._JWRB
   AXT(KIJS:KIJL) = 0._JWRB
   AYT(KIJS:KIJL) = 0._JWRB
   RLX(KIJS:KIJL) = 1._JWRB
   RLY(KIJS:KIJL) = 1._JWRB
   RNI(KIJS:KIJL) = 0._JWRB
   RMU(KIJS:KIJL) = 0._JWRB
   T1(KIJS:KIJL) = TMIN
   T2(KIJS:KIJL) = TMIN
!!!   WMDUR_TD = XNWVP*T2
   WMDX(KIJS:KIJL) = MAX(RLX(KIJS:KIJL),WVLMIN)
...
!     ----------------------------------------------------------
      PHIST(IJ) = MIN(ACF(IJ)/EMEAN(IJ), 1._JWRB)
      HMAX_N(IJ) = 0.5_JWRB*SQRT(1._JWRB-PHIST(IJ))*SQRT(Z0)*(1._JWRB+0.5_JWRB*GAMMA_E/Z0)*HS(IJ)
...
!     ----------------------------------------------------------------
      ...
      RN3 = ZPI*WMDX(IJ)*WMDY(IJ)*WMDUR_ST(IJ)*AXYT/(RLX(IJ)*RLY(IJ)*T2(IJ))
      RN2 = SQRT(ZPI)*(WMDX(IJ)*WMDUR_ST(IJ)/(RLX(IJ)*T2(IJ))*SQRT(1._JWRB-AXT(IJ)**2) &
&          +WMDX(IJ)*WMDY(IJ)/(RLX(IJ)*RLY(IJ))*SQRT(1._JWRB-AXY(IJ)**2) &
&          +WMDY(IJ)*WMDUR_ST(IJ)/(RLY(IJ)*T2(IJ))*SQRT(1._JWRB-AYT(IJ)**2))
      RN1 = WMDX(IJ)/RLX(IJ) + WMDY(IJ)/RLY(IJ) + WMDUR_ST(IJ)/T2(IJ)

!      IF (WMDX(IJ) /= 0._JWRB .AND. WMDY(IJ)/= 0._JWRB .AND. WMDUR_ST(IJ) /= 0._JWRB) THEN
         Z0 = W_MODE_ST(RN3, RN2, RN1)
!      ELSE IF (WMDUR_ST(IJ) == 0._JWRB) THEN

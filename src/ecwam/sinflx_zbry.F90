...
SUBROUTINE SINFLX_ZBRY (ICALL, NCALL, KIJS, KIJL,  &
 &                 LUPDTUS,                   &
 &                 FL1,                       &
...
REAL(KIND=JWRB), DIMENSION(NGST) :: UPROXYGST
...
!
  DO IGST=1,NGST
    ...
    CASE(0)
      UPROXYGST(IGST) = 32.0_JWRB * CDFAC * USTARGST(IJ,IGST) ! original, suggested by E. Rogers (2014) (young seas)
    CASE(1)
      UPROXYGST(IGST) = FRIC * CDFAC * USTARGST(IJ,IGST) ! following Komen et al. (1984) (developed seas) (FRIC=28)
    CASE(2)
      UPROXYGST(IGST) = UABSGST(IJ,IGST) * CDFAC ! because FRIC=1/sqrt(CD), then this turns to purely a wind dependence (USTARGST cancels out)
    END SELECT
  ENDDO
!
...
!/        complement one another. ------------------------------------ /
  ...
  &                 UPROXYGST(IGST)*CINV2*(ECOS2*COSU + ESIN2*SINU) - 1.0_JWRB)**2
!
...
!/        ww3_grid.inp: '&SIN6 SINA0 = 0.04 /' ----------------------- /
  DO IGST=1,NGST
    IF (SIN6A0.GT.0.0_JWRB) THEN
      W2(:,IGST)  = MIN( 0.0_JWRB,UPROXYGST(IGST) * CINV2* &
...
!
  DO IGST=1,NGST
    ...
    TAUWDIRGST(IJ,IGST)  = ATAN2(TAUWX(IGST),TAUWY(IGST))
...
! 9) --- Averaging over gust components ------------- /
  IGST=1
    ...
    UPROXYGST_AVG(IJ)   = UPROXYGST(IGST)
  ...
  DO IGST=2,NGST
    ...
    UPROXYGST_AVG(IJ)    = UPROXYGST_AVG(IJ)   + UPROXYGST(IGST)
...
!         spectral density of the wind input  ---------------------- /
  ...
  PHIWA(IJ) = CALCPHIWA(SPOSDENSIG,SNEGDENSIG,DSII) ! TODO: add the HiFreq contribution

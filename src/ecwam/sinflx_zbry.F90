...
INTEGER(KIND=JWIM) :: ITER, NFRE_EXT
...
REAL(KIND=JWRB), DIMENSION(KIJL)      :: SIG_N, SIG_U10, TAUWGST_AVG, TAUWDIRGST_AVG, USTARGST_AVG, UPROXYGST_AVG
REAL(KIND=JWRB), DIMENSION(KIJL,NGST) :: TAUWGST, TAUWDIRGST, UABSGST, USTARGST, Z0GST
...
IF (LLFACT) THEN ! TODO: how to make more efficient? is difficult...
  ...
  NFRE_EXT = CEILING(LOG(FRQMAX/(SIG(1)/ZPI))/LOG(FRATIO))+1
  ...
  &                      ROAIRN(IJ), SIG, DSII, NFRE_EXT, LFACT(IJ,:,IGST), TAUWX(IJ,IGST), TAUWY(IJ,IGST), TAU(IJ,IGST))
    ENDDO

...
! -------------------------------------------------------------------   
      ...
      REAL(KIND=JWRO), DIMENSION(NTOTIJ) :: NEMOTAUICX, NEMOTAUICY

! -------------------------------------------------------------------   
IF (LHOOK) CALL DR_HOOK('UPDNEMOSTRESS',0,ZHOOK_HANDLE)


      IF (LWNEMOCOU .AND. WAM2NEMO%LALLOC ) THEN
...
!$OMP     PARALLEL DO SCHEDULE(STATIC) PRIVATE(ICHNK, KIJS, IC, KIJL)
          DO ICHNK = 1, NCHNK
            ...
            NEMOTAUICX(KIJS:KIJL)   = WAM2NEMO%NEMOTAUICX(1:KIJL4CHNK(ICHNK), ICHNK) * ZNEMONTAUM1
            NEMOTAUICY(KIJS:KIJL)   = WAM2NEMO%NEMOTAUICY(1:KIJL4CHNK(ICHNK), ICHNK) * ZNEMONTAUM1
...
!$OMP     END PARALLEL DO
        ...
        ELSE
          ...
          NEMOTAUICX(1:NTOTIJ) = 0.0_JWRO
          NEMOTAUICY(1:NTOTIJ) = 0.0_JWRO
        ENDIF
...
!$OMP   PARALLEL DO SCHEDULE(STATIC) PRIVATE(ICHNK)
        DO ICHNK = 1, NCHNK
          ...
          WAM2NEMO%NEMOTAUICX(1:NPROMA_WAM,ICHNK) = 0.0_JWRO
          WAM2NEMO%NEMOTAUICY(1:NPROMA_WAM,ICHNK) = 0.0_JWRO
        ENDDO
!$OMP   END PARALLEL DO

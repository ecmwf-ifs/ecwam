...
! ----------------------------------------------------------------------
     ...
     &            LCIWACPL2,CITHRSH  ,CIBLOCK  ,LICETH     ,ALPFACX   , &
...
!       GET ISTREAM THAT CORRESPONDS TO IFS_STREAM
        CALL IGRIB_GET_VALUE(NGRIB_HANDLE_IFS2,'stream',IFS_STREAM)
...
!          -------------------------
      ...
      WRITE(IU06,*) '  '
      WRITE(IU06,*) '  NUMERICAL PRECISIONS:'
      WRITE(IU06,*) '  NPRECI = ', NPRECI
      WRITE(IU06,*) '  NPRECR = ', NPRECR
      WRITE(IU06,*) '  '
      ENDIF
     ...
     &      CDTW_LST(IC),' IS ',IDELWO_LST(IC),' SECS'
      ...
      WRITE(IU06,*) '   SHALLOW WATER TABLE NDEPTH = ', NDEPTH
...
!!!! PROPAGS2 has not yet been adapted for anything else then
      ...
      ELSEIF (IREFRA == 2) THEN
        ...
        IF (LLCFLCUROFF)  WRITE(IU06,*) ' IF CFL CRITERIA IS NOT SATISFIED, CURRENT REFRACTION IS TURNED OFF LOCALLY'
        IF (.NOT.LWCOU) THEN
...
!     ICE WATER DRAG COEFFICIENT
     ...
     &                    CITHRSH_TAIL
        ...
        ENDIF
        IF (LCISCAL)   WRITE (IU06,*) ' LINEAR SCALING OF SOURCE TERMS BASED ON SEA ICE COVER ACTIVATED'
        IF (LCIWACPL1) THEN
          WRITE (IU06,*) ' COUPLING THROUGH ICE BREAK UP AND ATTENUATION ACTIVATED'
          WRITE (IU06,*) ' ATTENUATION SCALED UP/DOWN BY FACTOR ALPFACX FOR SOLID/BROKEN ICE, ALPFACX= ', ALPFACX
        ENDIF
        IF (LCIWACPL2) WRITE (IU06,*) ' COUPLING THROUGH STRESS TO ICE ACTIVATED'
      ENDIF
     ...
     &   ,' TO THE FIELD DATA BASE'
      ...
      ELSEIF (IREST == 1) THEN
        IF (.NOT.LGRIBOUT) THEN
          WRITE(IU06,*) ' SPECTRA FILES WILL BE WRITTEN OUT TO DISK EVERY ... ',IDELRES,' SEC,'
          WRITE(IU06,*) ' AND AT THE END OF THE RUN.'
        ELSE
          WRITE(IU06,*) ' WAVE SPECTRA WILL BE DISPOSED EVERY ... ',IDELRES,' SEC.'
          WRITE(IU06,*) ' AND AT THE END OF THE RUN.'
        ENDIF

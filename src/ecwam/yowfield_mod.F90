...
MODULE YOWFIELD_MOD
   ...
   TYPE FORCING_FIELDS_FIELD
          ...
          REAL(KIND=JWRB), DIMENSION(:), POINTER :: USTRA=>NULL()
          REAL(KIND=JWRB), DIMENSION(:), POINTER :: VSTRA=>NULL()
          REAL(KIND=JWRB), DIMENSION(:), POINTER :: UCUR=>NULL()
          REAL(KIND=JWRB), DIMENSION(:), POINTER :: VCUR=>NULL()
          ...
          CLASS(FIELD_2RB), POINTER :: F_USTRA=>NULL()
          CLASS(FIELD_2RB), POINTER :: F_VSTRA=>NULL()
          CLASS(FIELD_2RB), POINTER :: F_UCUR=>NULL()
          CLASS(FIELD_2RB), POINTER :: F_VCUR=>NULL()
   ...
   CONTAINS
      ...
      SUBROUTINE FORCING_FIELDS_FIELD_INIT(SELF, UWND, VWND, AIRD, WSTAR, CICOVER, CITHICK, LKFR, USTRA, VSTRA, UCUR, VCUR, &
          ...
          REAL(KIND=JWRB), DIMENSION(:,:), INTENT(IN), OPTIONAL :: USTRA
          REAL(KIND=JWRB), DIMENSION(:,:), INTENT(IN), OPTIONAL :: VSTRA
          REAL(KIND=JWRB), DIMENSION(:,:), INTENT(IN), OPTIONAL :: UCUR
          REAL(KIND=JWRB), DIMENSION(:,:), INTENT(IN), OPTIONAL :: VCUR
          ...
          IF(PRESENT(USTRA)) CALL FIELD_NEW(SELF%F_USTRA, DATA=USTRA)
          IF(PRESENT(VSTRA)) CALL FIELD_NEW(SELF%F_VSTRA, DATA=VSTRA)
          IF(PRESENT(UCUR)) CALL FIELD_NEW(SELF%F_UCUR, DATA=UCUR)
          IF(PRESENT(VCUR)) CALL FIELD_NEW(SELF%F_VCUR, DATA=VCUR)
      ...
      SUBROUTINE FORCING_FIELDS_UPDATE_VIEW(SELF, BLOCK_INDEX)
          ...
          IF(ASSOCIATED(SELF%F_USTRA)) SELF%USTRA => SELF%F_USTRA%GET_VIEW(BLOCK_INDEX)
          IF(ASSOCIATED(SELF%F_VSTRA)) SELF%VSTRA => SELF%F_VSTRA%GET_VIEW(BLOCK_INDEX)
          IF(ASSOCIATED(SELF%F_UCUR)) SELF%UCUR => SELF%F_UCUR%GET_VIEW(BLOCK_INDEX)
          IF(ASSOCIATED(SELF%F_VCUR)) SELF%VCUR => SELF%F_VCUR%GET_VIEW(BLOCK_INDEX)
      ...
      SUBROUTINE FORCING_FIELDS_FINAL(SELF)
          ...
          ENDIF
          IF(ASSOCIATED(SELF%F_USTRA))THEN
              CALL FIELD_DELETE(SELF%F_USTRA)
          ENDIF
          IF(ASSOCIATED(SELF%F_VSTRA))THEN
              CALL FIELD_DELETE(SELF%F_VSTRA)
          ENDIF
          IF(ASSOCIATED(SELF%F_UCUR))THEN
              CALL FIELD_DELETE(SELF%F_UCUR)
          ENDIF

...
!.... Enable pinning of fields in page-locked memory
      ...
      IF (.NOT. WVPRPT%LALLOC)THEN
         CALL WVPRPT%ALLOC(WVPRPT, UBOUNDS=[NPROMA_WAM, NFRE, NCHNK])
      ENDIF

      IF (.NOT. FF_NOW%LALLOC) THEN
         CALL FF_NOW%ALLOC(FF_NOW, UBOUNDS=[NPROMA_WAM, NCHNK])
      ENDIF

      IF (.NOT. VARS_4D%LALLOC) THEN
        CALL VARS_4D%ALLOC(VARS_4D, UBOUNDS=[NPROMA_WAM, NANG, NFRE, NCHNK])
      ENDIF

      IF(.NOT. MIJ%LALLOC)THEN
        CALL MIJ%ALLOC(MIJ, UBOUNDS=[NPROMA_WAM, NCHNK])
      ENDIF

      IF (.NOT. INTFLDS%LALLOC) THEN
        CALL INTFLDS%ALLOC(INTFLDS, UBOUNDS=[NPROMA_WAM, NCHNK])
        INTFLDS%ALTWH(:,:)   = ZMISS
      ...
      IF (.NOT. FF_NEXT%LALLOC) THEN
         CALL FF_NEXT%ALLOC(FF_NEXT, UBOUNDS=[NPROMA_WAM,NCHNK])
      ENDIF


      IF (.NOT. WAM2NEMO%LALLOC) THEN
        CALL WAM2NEMO%ALLOC(WAM2NEMO, UBOUNDS=[NPROMA_WAM, NCHNK])
      ENDIF

      IF (.NOT. NEMO2WAM%LALLOC) THEN
        CALL NEMO2WAM%ALLOC(NEMO2WAM, UBOUNDS=[NPROMA_WAM, NCHNK])
      ENDIF

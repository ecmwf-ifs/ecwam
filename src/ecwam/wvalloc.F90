...
! ----------------------------------------------------------------------
      ...
      USE YOWSPEC  , ONLY : FF_NOW   ,FL1, XLLWS, MIJ
...
!         -------------------------
      ...
      IF (.NOT. FL1%LALLOC) THEN
        CALL FL1%ALLOC(UBOUNDS=[NPROMA_WAM, NANG, NFRE, NCHNK])
        FL1%PTR(:,:,:,:) = 0.0_JWRB
      ENDIF
      ...
      IF(.NOT. MIJ%LALLOC)THEN
        ...
        MIJ%PTR(:,:) = 0.0_JWRB
      ENDIF
      IF(.NOT. XLLWS%LALLOC)THEN
        CALL XLLWS%ALLOC(UBOUNDS=[NPROMA_WAM, NANG, NFRE, NCHNK])
        XLLWS%PTR(:,:,:,:) = 0.0_JWRB
      ENDIF

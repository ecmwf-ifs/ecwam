...
MODULE SNONLIN_CUF_PARAMETRISE_MOD
  ...
  ATTRIBUTES(DEVICE) SUBROUTINE SNONLIN_CUF_PARAMETRISE (KIJS, KIJL, FL1, FLD, SL, WAVNUM, DEPTH, AKMEAN, ENH, IJ)
    ...
    REAL(KIND=JWRB), INTENT(INOUT), DEVICE, DIMENSION(MLSTHG_D) :: ENH
    ...
    CASE (1)
      DO MC=1,NFRE_D
        ENH(MC) = MAX(MIN(ENH_MAX, TRANSF_CUF_PARAMETRISE(WAVNUM(IJ, MC), DEPTH(IJ))), ENH_MIN)
      END DO
      DO MC=NFRE_D + 1,MLSTHG_D
        XK = GM1_D*(ZPIFR_D(NFRE_D)*FRATIO**(MC - NFRE_D))**2
        ENH(MC) = MAX(MIN(ENH_MAX, TRANSF_CUF_PARAMETRISE(XK, DEPTH(IJ))), ENH_MIN)
      END DO
      
    CASE (2)
      CALL PEAK_ANG_CUF_PARAMETRISE(KIJS, KIJL, FL1, XNU, SIG_TH, IJ)
      DO MC=1,NFRE_D
        ENH(MC) = TRANSF_SNL_CUF_PARAMETRISE(WAVNUM(IJ, MC), DEPTH(IJ), XNU, SIG_TH)
      END DO
      DO MC=NFRE_D + 1,MLSTHG_D
        XK = GM1_D*(ZPIFR_D(NFRE_D)*FRATIO**(MC - NFRE_D))**2
        ENH(MC) = TRANSF_SNL_CUF_PARAMETRISE(XK, DEPTH(IJ), XNU, SIG_TH)
      END DO
    END SELECT

! utility program used to split the source code into all its individual
! parts (modules, programs, routines,...)
...
!
            ...
            if (ifst == 1 .AND. zlin (ifst:ifst) == 'c' .OR. &
                                zlin (ifst:ifst) == 'C' ) cycle
!
...
!
            if (zlin (llin:llin) == '&') then
               ifcnt1 = 1
               llin   = len_trim (zlin (1:llin-1))
            else
               ifcnt1 = 0
            endif
!
...
!  Skip label
!
         ilbl = verify (zstr (iwrd:lstr), zdgt)
         if (ilbl > 1) then
            lexp = lexp + ilbl - 1
            istr = iwrd - 1 + ilbl
            call expand
...
!  Skip bloc name
!
         iblk = index (zstr (iwrd:lstr), ':')
         if (iblk > 0) then
            if (verify (zstr (iwrd:iblk-1), zdgt//zlwc//zupc) == 0) then
               lexp = lexp + iblk - 1
               istr = iwrd - 1 + iblk + 1
               call expand
            endif
...
contains
   subroutine expand
!
!  Search until first non-blank, non-tab
!
         iwrd = verify (zstr (istr:lstr), ztab//' ') + istr - 1
!
!  Expand each TAB on to next tab mark
!
         iwhi = istr
         do
            itab = index (zstr (iwhi:iwrd-1), ztab)
            if (itab == 0) then
               lexp = lexp + iwrd - iwhi
               exit
            endif
            iwhi = iwhi + itab
            lfil = min (lexp + itab, linem)
...
use splitdefs
use splitcurs
character (len=lstr), intent (inout) :: zstr  ! The line
integer, intent (inout)              :: lstr  ! its trimmed length
! ____________________________________________________________________
!
...
!
      ...
      if (index (zstr (1:lstr), ztab) == 0) return
      if (verify (zstr (1:lstr), ztab//' ') == 0) then
         lstr = 0
         return
      endif
...
!
         ...
         zlinw (lexp+1:linem) = zstr (iwrd:lstr)
   end subroutine expand

...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(KIJS, IJSB, KIJL, IJLB, IJ, JH)
  DO ICHNK = 1, NCHNK
    ...
    BLK2LOC(KIJS:KIJL, ICHNK)%IFROMIJ = BLK2GLO(IJSB:IJLB)%IXLG
    BLK2LOC(KIJS:KIJL, ICHNK)%KFROMIJ = BLK2GLO(IJSB:IJLB)%KXLT
    BLK2LOC(KIJS:KIJL, ICHNK)%JFROMIJ = NGY - BLK2GLO(IJSB:IJLB)%KXLT + 1
    ...
    NYE(ICHNK) = 1
    DO IJ = KIJS, KIJL
      NXS(ICHNK) = MIN(NXS(ICHNK), BLK2LOC(IJ, ICHNK)%IFROMIJ)  
      NXE(ICHNK) = MAX(NXE(ICHNK), BLK2LOC(IJ, ICHNK)%IFROMIJ)  
      NYS(ICHNK) = MIN(NYS(ICHNK), BLK2LOC(IJ, ICHNK)%JFROMIJ)  
      NYE(ICHNK) = MAX(NYE(ICHNK), BLK2LOC(IJ, ICHNK)%JFROMIJ)  
    ENDDO

    IF (LLWVENVI) THEN
      DO IJ = KIJS, KIJL
        JH = BLK2LOC(IJ,ICHNK)%KFROMIJ
        WVENVI(IJ, ICHNK)%COSPHM1 = 1.0_JWRB/COSPH(JH) 
        WVENVI(IJ, ICHNK)%DELLAM1 = 1.0_JWRB/DELLAM(JH) 
      ENDDO
    ENDIF
 
    IF (LLWVENVI) THEN
      WVENVI(KIJS:KIJL, ICHNK)%DEPTH = DEPTH_INPUT(IJSB:IJLB)

!!!!     when this is moved to reading depth as an input field, wvenvi should be allocated and intialised in wvalloc !!!
      WVENVI(KIJS:KIJL, ICHNK)%UCUR = 0.0_JWRB
      WVENVI(KIJS:KIJL, ICHNK)%VCUR = 0.0_JWRB
    ENDIF

!!! these are fictious points but will point to the first point in the chunk as it should always exist 
    IF (KIJL < NPROMA_WAM) THEN
      BLK2LOC(KIJL+1:NPROMA_WAM, ICHNK)%IFROMIJ = BLK2LOC(1, ICHNK)%IFROMIJ
      BLK2LOC(KIJL+1:NPROMA_WAM, ICHNK)%KFROMIJ = BLK2LOC(1, ICHNK)%KFROMIJ
      BLK2LOC(KIJL+1:NPROMA_WAM, ICHNK)%JFROMIJ = BLK2LOC(1, ICHNK)%JFROMIJ

      IF (LLWVENVI) THEN
        WVENVI(KIJL+1:NPROMA_WAM, ICHNK)%COSPHM1 = WVENVI(1, ICHNK)%COSPHM1 
        WVENVI(KIJL+1:NPROMA_WAM, ICHNK)%DELLAM1 = WVENVI(1, ICHNK)%DELLAM1 
 
        WVENVI(KIJL+1:NPROMA_WAM, ICHNK)%DEPTH = WVENVI(1, ICHNK)%DEPTH 
        WVENVI(KIJL+1:NPROMA_WAM, ICHNK)%UCUR = WVENVI(1, ICHNK)%UCUR
        WVENVI(KIJL+1:NPROMA_WAM, ICHNK)%VCUR = WVENVI(1, ICHNK)%VCUR 
      ENDIF
    ENDIF
...
!$OMP   END PARALLEL DO
  ...
  WRITE(IU06,*) ' MODEL DECOMPOSITION NXFFS_LOC = ', NXFFS_LOC
  WRITE(IU06,*) ' MODEL DECOMPOSITION NXFFE_LOC = ', NXFFE_LOC
  WRITE(IU06,*) ' MODEL DECOMPOSITION NYFFS_LOC = ', NYFFS_LOC
  WRITE(IU06,*) ' MODEL DECOMPOSITION NYFFE_LOC = ', NYFFE_LOC

...
! -------------------------------------------------------------------
     ...
     &            IJSLOC   ,IJLLOC   ,IJGLOBAL_OFFSET, IJFROMBLOC,      &
     &            DELLAM   ,COSPH    ,DELPHI   ,                        &
     &            CDR      ,SDR      ,PRQRT
...
!! CREATE THE NPROMA BLOCKS STRUCTURES
!  -----------------------------------
! ADJUST NPROMA_WAM
MTHREADS=1
...
!!!? debile: consider an openMP loop here
  DO IBLOC = 1, NBLOC
    DO IPRM = 1, NPROMA_WAM 
      IJ = IJFROMBLOC(IPRM,IBLOC)
      IF (IJ > 0) THEN
        ...
        JH = BLK2LOC(IPRM,IBLOC)%KFROMIJ
        WVENVI(IPRM,IBLOC)%COSPHM1 = 1.0_JWRB/COSPH(JH) 
        WVENVI(IPRM,IBLOC)%DELLAM1 = 1.0_JWRB/DELLAM(JH) 
...
!!!!     when this is moved to reading depth as an input field, wvenvi should be allocated and intialised in wvalloc !!!
      ...
      ELSE
        BLK2LOC(IPRM,IBLOC)%IFROMIJ=1
        BLK2LOC(IPRM,IBLOC)%KFROMIJ=1
        BLK2LOC(IPRM,IBLOC)%JFROMIJ=NGY
        JH = BLK2LOC(IPRM,IBLOC)%KFROMIJ
        WVENVI(IPRM,IBLOC)%COSPHM1 = 1.0_JWRB
        WVENVI(IPRM,IBLOC)%DELLAM1 = 1.0_JWRB/DELLAM(JH) 
 
        WVENVI(IPRM,IBLOC)%DEPTH = BATHYMAX
        WVENVI(IPRM,IBLOC)%UCUR = 0.0_JWRB
        WVENVI(IPRM,IBLOC)%VCUR = 0.0_JWRB
      ENDIF

...
ELSE
  ...
  NXFFS_LOC = NGX  ! initialisation (see below)
  NXFFE_LOC = 1
  NYFFS_LOC = NGY
  NYFFE_LOC = 1
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(KIJS, IJSB, KIJL, IJLB, IJ, JH)
  DO ICHNK = 1, NCHNK
    ...
    DO IJ = KIJS, KIJL
      NXFFS_LOC = MIN(NXFFS_LOC, BLK2LOC(IJ, ICHNK)%IFROMIJ)  
      NXFFE_LOC = MAX(NXFFE_LOC, BLK2LOC(IJ, ICHNK)%IFROMIJ)  
      NYFFS_LOC = MIN(NYFFS_LOC, BLK2LOC(IJ, ICHNK)%JFROMIJ)  
      NYFFE_LOC = MAX(NYFFE_LOC, BLK2LOC(IJ, ICHNK)%JFROMIJ)  
    ENDDO

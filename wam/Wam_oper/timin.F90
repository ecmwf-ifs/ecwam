...
#include "incdate.intfb.h"
     ...
     &               ROAIRO, ZIDLOLD, CICOVER, CITHICK
      ...
      REAL(KIND=JWRB), DIMENSION(IJS:IJL) :: U10,US,DS,ADS,ZIDL,CICR,CITH,CD
      REAL(KIND=JWRB), DIMENSION(IJS:IJL) :: U102,US2,DS2,ADS2,ZIDL2,CICR2,CITH2
      REAL(KIND=JWRB), DIMENSION(IJS:IJL) :: U103,US3, DS3,ADS3,ZIDL3,CICR3,CITH3
...
!$OMP     PARALLEL DO SCHEDULE(STATIC) PRIVATE(JKGLO,KIJS,KIJL,IJ,D)
          DO JKGLO=IJS,IJL,NPROMA
            ...
            DO IJ=KIJS,KIJL
              ...
              D = DS2(IJ)-DS(IJ)
              IF (ABS(D) > PI) D = D-ZPI*SIGN(1.0_JWRB,D)

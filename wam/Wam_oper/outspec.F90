...
!     -------------------
      ...
      IF (LWCOU .AND. LIFS_IO_SERV_ENABLED .AND. LWAM_USE_IO_SERV) THEN
        IF (.NOT.ASSOCIATED(OUTWSPEC_IO_SERV_CALLBACK)) CALL ABOR1('OUTWSPEC_IO_SERV_CALLBACK IS NOT INITIALIZED')
        CALL OUTWSPEC_IO_SERV_CALLBACK(SPEC, MARSTYPE, CDATE, IFCST)
      ELSE

SUBROUTINE SAVSTRESS(IJS, IJL, WVENVI, FF_NOW, NBLKS, NBLKE, CDTPRO, CDATEF)
! ----------------------------------------------------------------------
...
! ----------------------------------------------------------------------
      ...
      USE YOWGRID  , ONLY : IJSLOC   ,IJLLOC   ,IJGLOBAL_OFFSET, NPROMA_WAM, NBLOC
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JKGLO,KIJS,KIJL,IJ)
      DO JKGLO=MIJS,MIJL,NPROMA
        ...
        DO IJ=KIJS+IJOFFSET,KIJL+IJOFFSET
          ...
          RFIELD(IJ,7) = Z0B(IJ)
          RFIELD(IJ,8) = AIRD(IJ)
          RFIELD(IJ,9) = WSTAR(IJ)
          RFIELD(IJ,10) = CICOVER(IJ)
          RFIELD(IJ,11) = CITHICK(IJ)
          RFIELD(IJ,12) = UCUR(IJ) 
          RFIELD(IJ,13) = VCUR(IJ) 
        ENDDO
      ENDDO
!$OMP END PARALLEL DO

...
!       SIGNIFICANT WAVE HEIGHT CONVERSION
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=4._JWRB*SQRT(MAX(EM(KIJS:KIJL),0._JWRB))
      ENDIF
...
!       CONVERSION TO PERIOD
      ...
      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=USNEW(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
!       CONVERT DIRECTIONS TO DEGREES AND METEOROLOGICAL CONVENTION
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=MOD(DEG*THWNEW(KIJS:KIJL)+180._JWRB,360._JWRB)
      ENDIF
...
!!      velocity is determined from U10 and CD.
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=MAX(USNEW(KIJS:KIJL)**2,EPSUS)/MAX(U10NEW(KIJS:KIJL)**2,EPSU10)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=TAUW(KIJS:KIJL)/MAX(USNEW(KIJS:KIJL)**2,EPSUS)
      ENDIF
      ...
      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=U10NEW(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
!       WINDSEA SIGNIFICANT WAVE HEIGHT CONVERSION
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=4._JWRB*SQRT(MAX(ESEA(KIJS:KIJL),0._JWRB))
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
!       TOTAL SWELL SIGNIFICANT WAVE HEIGHT CONVERSION
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=4._JWRB*SQRT(MAX(ESWELL(KIJS:KIJL),0._JWRB))
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
!       CONVERT DIRECTIONS TO DEGREES AND METEOROLOGICAL CONVENTION
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=MOD(DEG*THWISEA(KIJS:KIJL)+180._JWRB,360._JWRB)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
!       CONVERT DIRECTIONS TO DEGREES AND METEOROLOGICAL CONVENTION
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=MOD(DEG*THSWELL(KIJS:KIJL)+180._JWRB,360._JWRB)
      ENDIF
...
!       CONVERSION TO PERIOD
      ...
      IF(IPFGTBL(IR).NE.0) THEN
        ...
        ELSE
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=ALTWH(KIJS:KIJL)
        ENDIF
      ENDIF
      ...
      IF(IPFGTBL(IR).NE.0) THEN
        IF(.NOT.ALLOCATED(CALTWH)) THEN
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=ZMISS
        ELSE
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=CALTWH(KIJS:KIJL)
        ENDIF
      ENDIF
      ...
      IF(IPFGTBL(IR).NE.0) THEN
        IF(.NOT.ALLOCATED(RALTCOR)) THEN
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=ZMISS
        ELSE
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=RALTCOR(KIJS:KIJL)
        ENDIF
      ENDIF
      ...
      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=P1SEA(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=P1SWELL(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=P2SEA(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=P2SWELL(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=SPRDSEA(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=SPRDSWELL(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=C4(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=BF(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=QP(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=DEPTH(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=HMAX(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=TMAX(KIJS:KIJL)
      ENDIF

!     SURFACE STOKES DRIFT U and V
      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=USTOKES(KIJS:KIJL)
      ENDIF
      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=VSTOKES(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        IF(.NOT.ALLOCATED(U)) THEN
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=0._JWRB
        ELSE
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=U(KIJS:KIJL)
        ENDIF
      ENDIF
      ...
      IF(IPFGTBL(IR).NE.0) THEN
        IF(.NOT.ALLOCATED(V)) THEN
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=0._JWRB
        ELSE
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=V(KIJS:KIJL)
        ENDIF
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=PHIEPS(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=PHIAW(KIJS:KIJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        GBOUT(KIJS:KIJL,ITOBOUT(IR))=TAUOC(KIJS:KIJL)
      ENDIF

      DO ITR=1,NTRAIN
        IR=IR+1
        IF(IPFGTBL(IR).NE.0) THEN
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=4._JWRB*SQRT(MAX(EMTRAIN(KIJS:KIJL,ITR),0._JWRB))
        ENDIF

        IR=IR+1
        IF(IPFGTBL(IR).NE.0) THEN
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=MOD(DEG*THTRAIN(KIJS:KIJL,ITR)+180._JWRB,360._JWRB)
        ENDIF

        IR=IR+1
        IF(IPFGTBL(IR).NE.0) THEN
          GBOUT(KIJS:KIJL,ITOBOUT(IR))=PMTRAIN(KIJS:KIJL,ITR)
        ENDIF
      ENDDO

...
!     PREPARE THE WAVE SPECTRA THAt SHOULD BE USED FOR OUTPUT
      ...
      IF (IREFRA == 2 .OR. IREFRA == 3) THEN
        CALL INTPOL (KIJS, KIJL, FL1, FL2ND, WAVNUM, IRA)
      ELSE
...
!!      velocity is determined from U10 and CD.
      ...
      IR=IR+1
      IF (IPFGTBL(IR) /= 0) THEN
        CALL MEANSQS (XKMSS_CUTOFF, KIJS, KIJL, FL1, WAVNUM, U10NEW(KIJS), USNEW(KIJS), THWNEW(KIJS), BOUT(KIJS,ITOBOUT(IR)))
      ENDIF
...
!     SURFACE STOKES DRIFT U and V
      ...
      IF (IPFGTBL(IR) /= 0) THEN
        IF (.NOT.ALLOCATED(U)) THEN
          BOUT(KIJS:KIJL,ITOBOUT(IR))=0._JWRB
        ELSE
          BOUT(KIJS:KIJL,ITOBOUT(IR))=U(KIJS:KIJL)
        ENDIF
      ENDIF
      ...
      IF (IPFGTBL(IR) /= 0) THEN
        IF (.NOT.ALLOCATED(V)) THEN
          BOUT(KIJS:KIJL,ITOBOUT(IR))=0._JWRB
        ELSE
          BOUT(KIJS:KIJL,ITOBOUT(IR))=V(KIJS:KIJL)
        ENDIF
      ENDIF
...
!!!for testing
        CALL HALPHAP(KIJS, KIJL, USNEW(KIJS), THWNEW(KIJS), FL1, HALP)
...
!!!for testing
        ...
        CALL MEANSQS (XMODEL_CUTOFF, KIJS, KIJL, FL1, WAVNUM, U10NEW(KIJS), USNEW(KIJS), THWNEW(KIJS), BOUT(KIJS,ITOBOUT(IR)))
      ENDIF
...
!     APPLY SEA ICE MASK AND SEA MASK IF NECESSARY
      CALL OUTSETWMASK (KIJS, KIJL, IODP(KIJS), CICOVER(KIJS),BOUT)

...
!     PREPARE THE WAVE SPECTRA THAt SHOULD BE USED FOR OUTPUT
      ...
      IF (IREFRA.EQ.2 .OR. IREFRA.EQ.3) THEN
        CALL INTPOL (FL1, FL2ND, IJS, IJL, IRA)
      ELSE
        FL2ND(IJS:IJL,:,:) = FL1(IJS:IJL,:,:)
      ENDIF
...
!!     velocity is determined from U10 and CD.
      ...
      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        BOUT(IJS:IJL,ITOBOUT(IR))=TAUW(IJS:IJL)/MAX(USNEW(IJS:IJL)**2,EPSUS)
      ENDIF
...
!     SURFACE STOKES DRIFT U and V
      ...
      IF(IPFGTBL(IR).NE.0) THEN
        ...
        ELSE
          BOUT(IJS:IJL,ITOBOUT(IR))=U(IJS:IJL)
        ENDIF
      ENDIF
      ...
      IF(IPFGTBL(IR).NE.0) THEN
        IF(.NOT.ALLOCATED(V)) THEN
          BOUT(IJS:IJL,ITOBOUT(IR))=0._JWRB
        ELSE
          BOUT(IJS:IJL,ITOBOUT(IR))=V(IJS:IJL)
        ENDIF
      ENDIF
      ...
      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        BOUT(IJS:IJL,ITOBOUT(IR))=CICOVER(IJS:IJL)
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        BOUT(IJS:IJL,ITOBOUT(IR))=CITHICK(IJS:IJL)
      ENDIF
...
!     APPLY SEA ICE MASK AND SEA MASK IF NECESSARY
      CALL OUTSETWMASK (IJS, IJL, IODP(IJS), CICOVER(IJS),BOUT)

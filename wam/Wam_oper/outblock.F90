...
! ----------------------------------------------------------------------
     ...
     &            ROAIRN   ,ZIDLNEW
      ...
      USE YOWPCONS , ONLY : ZMISS    ,DEG      ,EPSU     ,EPSU10
...
!     COMPUTE MEAN PARAMETERS
      ...
      CALL KURTOSIS(FL3,IJS,IJL,IG,C3,C4,BF,QP,FP,HMAX,TMAX)
...
!       CONVERSION TO PERIOD
      ...
      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        CALL WDIRSPREAD (FL3,IJS,IJL,LLPEAKF,BOUT(IJS,ITOBOUT(IR)))
      ENDIF
...
!     APPLY SEA ICE MASK AND SEA MASK IF NECESSARY
      CALL OUTSETWMASK (IJS, IJL, IODP(IJS), CICOVER(IJS,IG),BOUT(IJS,:))

...
!            and in the absolute frame of reference if currents are used 
      REAL(KIND=JWRB), DIMENSION(IJS:IJL,NANG,NFRE) :: FL2ND
...
!     PREPARE THE WAVE SPECTRA THAt SHOULD BE USED FOR OUTPUT
      ...
      ELSE
        FL2ND(:,:,:) = FL1(:,:,:)
      ENDIF
      IF(LSECONDORDER) CALL CAL_SECOND_ORDER_SPEC(FL2ND,IJS,IJL,DPTH,SIG)


!     COMPUTE MEAN PARAMETERS
      CALL FEMEAN (FL2ND, IJS, IJL, EM, FM)
...
!       SIGNIFICANT WAVE HEIGHT CONVERSION
      ...
      IF(IPFGTBL(IR).NE.0) THEN
        ITG=ITOBOUT(IR)
        CALL STHQ (FL2ND, IJS, IJL, BOUT(IJS,ITG))
...
! for testing: estimate of wave age based on mean frequency
            WAVEAGE(IJ)=MIN(GOZPI/(0.9_JWRB*FM(IJ)*MAX(USNEW(IJ),EPSUS)),100.0_JWRB)
          ELSE
...
! for testing: estimate of wave age based on wind mean frequency
            WAVEAGESEA(IJ)=MIN(GOZPI/(0.9_JWRB*FSEA(IJ)*MAX(USNEW(IJ),EPSUS)),100.0_JWRB)
          ELSE
            BOUT(IJ,ITOBOUT(IR))=ZMISS
! for testing: estimate of wave age based on wind mean frequency
            WAVEAGESEA(IJ)=ZMISS
...
!       CONVERSION TO PERIOD
      ...
      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        CALL MWP1 (FL2ND, IJS, IJL, BOUT(IJS,ITOBOUT(IR)))
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        CALL MWP2 (FL2ND, IJS, IJL, BOUT(IJS,ITOBOUT(IR)))
      ENDIF

      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        CALL WDIRSPREAD (FL2ND, IJS, IJL, EM, LLPEAKF, BOUT(IJS,ITOBOUT(IR)))
      ENDIF
...
!     SURFACE STOKES DRIFT U and V
      ...
      IR=IR+1
      IF(IPFGTBL(IR).NE.0) THEN
        CALL SE10MEAN (FL2ND, IJS, IJL, FLD1(IJS))
...
!       CONVERT DIRECTIONS TO DEGREES AND METEOROLOGICAL CONVENTION
      ...
      DO IH=1,NTEWH
        IR=IR+1
        IF(IPFGTBL(IR).NE.0) THEN
          CALL SEBTMEAN (FL2ND, IJS, IJL, TEWH(IH-1), TEWH(IH), BOUT(IJS,ITOBOUT(IR)))
!         SIGNIFICANT WAVE HEIGHT CONVERSION

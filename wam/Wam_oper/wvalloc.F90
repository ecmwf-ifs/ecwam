...
!         -------------------------
      ...
      IF (.NOT.ALLOCATED(WVPRPT)) ALLOCATE(WVPRPT(NPROMA_WAM,NFRE,NBLOC))

      IF (.NOT.ALLOCATED(FF_NOW)) ALLOCATE(FF_NOW(NPROMA_WAM,NBLOC))

      IF (.NOT.ALLOCATED(FL1)) THEN
        ALLOCATE(FL1(NPROMA_WAM,NANG,NFRE,NBLOC))
        FL1(:,:,:,:) = 0.0_JWRB
      ENDIF

      IF (.NOT.ALLOCATED(INTFLDS)) THEN 
        ALLOCATE(INTFLDS(NPROMA_WAM,NBLOC))
      ...
      IF (.NOT.ALLOCATED(WAM2NEMO)) THEN
        ALLOCATE(WAM2NEMO(NPROMA_WAM,NBLOC))
      ...
      IF (.NOT.ALLOCATED(NEMO2WAM)) THEN
        ALLOCATE(NEMO2WAM(NPROMA_WAM,NBLOC))

...
!     IFS IO SERVER ENABLED
      ...
      REAL(KIND=JWRB), DIMENSION(IJS:IJL) :: BETAB
...
!       UPDATE ANALYSIS TIME
        ...
        IF (NASS > 0 ) THEN
          IF ( CDTPRO == CDTASS ) THEN
            CALL WAMASSI (LDSTOP, LDWRRE, FF_NOW, INTFLDS)
          ENDIF
...
!         8. WINDSEA MEAN FREQUENCY
        ...
        NPROMA=NPROMA_WAM
        CALL GSTATS(1443,0)
!$OMP   PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(JKGLO,KIJS,KIJL,IFLDOFFSET,IFLD)
        DO JKGLO=IJS,IJL,NPROMA
          KIJS=JKGLO
          KIJL=MIN(KIJS+NPROMA-1,IJL)
          CALL OUTBETA (KIJS, KIJL, PRCHAR, FF_NOW(KIJS), BETAB(KIJS))
...
!         IT MIGHT ALSO BE USED FOR NEMO !!!!!!!!!! 
          ...
          IF (LWSTOKES) THEN
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(KIJS:KIJL,IFLDOFFSET)=INTFLDS(KIJS:KIJL)%USTOKES
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(KIJS:KIJL,IFLDOFFSET)=INTFLDS(KIJS:KIJL)%VSTOKES
          ENDIF

          IF (LWFLUX) THEN
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(KIJS:KIJL,IFLDOFFSET)=INTFLDS(KIJS:KIJL)%PHIOCD
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(KIJS:KIJL,IFLDOFFSET)=INTFLDS(KIJS:KIJL)%TAUOCXD
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(KIJS:KIJL,IFLDOFFSET)=INTFLDS(KIJS:KIJL)%TAUOCYD
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(KIJS:KIJL,IFLDOFFSET)=INTFLDS(KIJS:KIJL)%WSEMEAN
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(KIJS:KIJL,IFLDOFFSET)=INTFLDS(KIJS:KIJL)%WSFMEAN
          ENDIF

          DO IFLD=IFLDOFFSET+1,NWVFIELDS
            WVBLOCK(KIJS:KIJL,IFLD)=0.0_JWRB
          ENDDO

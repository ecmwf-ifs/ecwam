...
!     TEST471 (modified SWELLF):
      ...
      REAL(KIND=JWRB), DIMENSION(IJS:IJL,NGST) :: XSTRESS_INC, YSTRESS_INC
...
! Initialisation
      ...
      DO IGST=1,NGST
        DO IJ=IJS,IJL
          XSTRESS_INC(IJ,IGST)=0.0_JWRB
          XSTRESS(IJ,IGST)=0.0_JWRB
          YSTRESS_INC(IJ,IGST)=0.0_JWRB
...
!         ---------------------------------------------------------
        ...
        DO K=1,NANG
          DO IJ=IJS,IJL
            CONST11(IJ)=0.5_JWRB*CONSTF(IJ)*SINTH(K)
            CONST22(IJ)=0.5_JWRB*CONSTF(IJ)*COSTH(K)
          ENDDO
          ...
          DO IGST=1,NGST
            DO IJ=IJS,IJL
              XSTRESS(IJ,IGST)=XSTRESS(IJ,IGST)+XSTRESS_INC(IJ,IGST)
              YSTRESS(IJ,IGST)=YSTRESS(IJ,IGST)+YSTRESS_INC(IJ,IGST)

              SLP(IJ,IGST) = SLP(IJ,IGST)*F(IJ,K,M)

              XSTRESS_INC(IJ,IGST)=SLP(IJ,IGST)*CONST11(IJ)
              YSTRESS_INC(IJ,IGST)=SLP(IJ,IGST)*CONST22(IJ)

...
! ----------------------------------------------------------------------
     ...
     &            LLNEWCURR,NORMS_IN ,LLWSWAVE ,LLWDWAVE
...
!__PACK
      ...
      IF (.NOT.ALLOCATED(NORMS_IN)) ALLOCATE(NORMS_IN(NFIELDS,4))
      NORMS_IN(:,1)=0.0_JWRB
      NORMS_IN(:,2)=1.0_JWRB
      NORMS_IN(:,3)=-1.0_JWRB
      NORMS_IN(:,4)=-1.0_JWRB
...
!     BUILD DEPTH POINTER AND RESET DEPTH TO ALL POSITIVE
      ...
      IF (.NOT.ALLOCATED(IOBND)) ALLOCATE(IOBND(NINF-1:NSUP))
      IF (.NOT. LLUNSTR) THEN
         IOBND(:)=1
      ELSE
        DO IJ=NINF,NSUP
...
!         ------------
      ...
      CALL GETSPEC(FL1,NBLKS,NBLKE,IREAD)

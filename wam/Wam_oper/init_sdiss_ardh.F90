...
!     l(k,th)=1/(2*pi²)= the breaking crest density
      ...
      ALLOCATE(INDICESSAT(NSDSNTH*2+1, NANG))
      IF (ALLOCATED(SATWEIGHTS)) DEALLOCATE(SATWEIGHTS)
      ALLOCATE(SATWEIGHTS(NSDSNTH*2+1, NANG))
      ...
      DO K=1,NANG
        DO I_INT=K-NSDSNTH, K+NSDSNTH
          ...
          INDICESSAT(I_INT-(K-NSDSNTH)+1,K) = J_INT
          ...
          ELSE
            DELTH_LOC=DELTH
          ENDIF
          SATWEIGHTS(I_INT-(K-NSDSNTH)+1, K) = DELTH_LOC*COS(TH(K)-TH(J_INT))**ISB
        END DO
      END DO

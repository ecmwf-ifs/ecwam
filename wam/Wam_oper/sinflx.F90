...
! ----------------------------------------------------------------------
      ...
      USE YOWPHYS  , ONLY : BMAXOKAPDTH, DTHRN_A, DTHRN_U 
...
IF(LLNORMAGAM .AND. LLCAPCHNK ) THEN
  RNFAC(:) = BMAXOKAPDTH * ( 1.0_JWRB+DTHRN_A*(1.0_JWRB+TANH(U10NEW(:)-DTHRN_U)) )
ELSE
  RNFAC(:) = BMAXOKAPDTH
ENDIF
...
IF(LUPDTUS) THEN
  ...
  CALL AIRSEA (FL1, U10NEW, THWNEW, ROAIRN, TAUW, TAUWDIR, USNEW, Z0NEW, IJS, IJL, ICODE_WND, IUSFG) 
ENDIF
...
CALL SINPUT (NGST, FL1, FL, IJS, IJL, THWNEW, USNEW, Z0NEW, ROAIRN, WSTAR, RNFAC, SL, SPOS, XLLWS) 
...
! UPDATE TAUW
CALL STRESSO (FL1, SL, SPOS, IJS, IJL, THWNEW, USNEW, Z0NEW, ROAIRN, TAUW, TAUWDIR, PHIWA, LLPHIWA) 

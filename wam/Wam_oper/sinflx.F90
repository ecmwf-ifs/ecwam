...
INTEGER(KIND=JWIM), INTENT(IN) :: IJS, IJL !! 1st DIMENSION of GFL and GXLLWS.  
...
IF(LUPDTUS) THEN
  ...
  IF(ICALL == 1 ) GFL(KIJS:KIJL,:,NFRE) = MAX(GFL(KIJS:KIJL,:,NFRE),FLM(:,:))

  CALL AIRSEA (GFL, U10NEW, THWNEW, ROAIRN, TAUW, TAUWDIR, RNFAC, USNEW, Z0NEW, Z0B, KIJS, KIJL, ICODE_WND, IUSFG) 
ENDIF
...
CALL SINPUT (NGST, GFL, FLD, KIJS, KIJL, THWNEW, U10NEW, USNEW, Z0NEW, ROAIRN, WSTAR, RNFAC, SL, SPOS, GXLLWS) 
...
! UPDATE TAUW
CALL STRESSO (GFL, SL, SPOS, KIJS, KIJL, THWNEW, USNEW, Z0NEW, ROAIRN, RNFAC, TAUW, TAUWDIR, PHIWA, LLPHIWA) 

! INSURE MINIMUN ENERGY AT THE LAST FREQUENCY BIN.
IF(ICALL < NCALL ) CALL FLMINTAIL(KIJS, KIJL, U10NEW, THWNEW, USNEW, FMEANWS, GFL) 

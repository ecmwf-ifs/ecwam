SUBROUTINE SINFLX (ICALL, IJS, IJL, &
&                  LUPDTUS, &
...
REAL(KIND=JWRB), DIMENSION(IJS:IJL), INTENT(IN) :: WSTARNEW !! FREE CONVECTION VELOCITY SCALE (M/S)
...
REAL(KIND=JWRB), DIMENSION(IJS:IJL), INTENT(OUT) :: Z0NEW  !! ROUGHNESS LENGTH IN M.
...
ELSE
  ...
  ICODE_WND = 3
ENDIF
IF(LUPDTUS) CALL AIRSEA (FL1, U10NEW, THWNEW, ROAIRN, TAUW, USNEW, Z0NEW, IJS, IJL, ICODE_WND, IUSFG) 

! COMPUTE WIND INPUT
!!  FL AND SL ARE INITIALISED IN SINPUT
CALL SINPUT (FL1, FL, IJS, IJL, THWNEW, USNEW, Z0NEW, ROAIRN, WSTARNEW, SL, SPOS, XLLWS) 

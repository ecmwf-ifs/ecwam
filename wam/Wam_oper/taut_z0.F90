...
#include "stress_gc.intfb.h"
      ...
      REAL(KIND=JWRB), PARAMETER :: Z0MIN = 0.0000001_JWRB
...
IF (LHOOK) CALL DR_HOOK('TAUT_Z0',0,ZHOOK_HANDLE)
      ...
      DO IJ = IJS, IJL
        TAUWEFF(IJ) =  MIN(TAUW(IJ)*US2TOTAUW, (-0.21339_JWRB + 0.093698_JWRB*UTOP(IJ) -0.0020944_JWRB*UTOP(IJ)**2 + 5.5091E-5_JWRB*UTOP(IJ)**3)**2 )
      ENDDO
...
!     COMPUTE THE PHILLIPS PARAMETER (only in the wind direction)
      ...
      DO K = 1, NANG
        ...
        ENDDO
      ENDDO
      HALPHAP(:) = 0.5_JWRB*MIN(ALPHAP(:), ALPHAPMAX)
      ...
      DO IJ = IJS, IJL
        ...
        USTOLD = USTAR(IJ)
        TAUOLD = USTOLD**2
        ANG_GC = MAX(MIN(0.03_JWRB*UTOP(IJ), 0.68_JWRB),0.1_JWRB)

...
#include "stress_gc.intfb.h"
      ...
      REAL(KIND=JWRB) :: US2TOTAUW, TAUWTOUS2
      REAL(KIND=JWRB) :: TAUWFG
...
!     COMPUTE THE PHILLIPS PARAMETER (only in the wind direction)
      ...
      DO IJ = IJS, IJL
        ...
        TAUWFG = (1-IUSFG)*MIN(TAUW(IJ), TAUWTOUS2*USTOLD**2) + IUSFG*TAUW(IJ)
        TAUOLD = MAX(USTOLD**2,TAUWFG*US2TOTAUW)
...
!         TOTAL kinematic STRESS:
          TAUNEW = US2TOTAUW*TAUWFG + TAUV + TAUUNR(IJ)
          USTNEW = SQRT(TAUNEW)
...
!!debile
       ...
       write(IU06,*) 'stress ',TAUNEW, TAUWFG , TAUV , TAUUNR(IJ), ALPHAP(IJ)
       CALL FLUSH(IU06)

...
!                       UTOP, UDIR, WSTAR, TAUW, TAUWDIR, RNFAC,
!                       USTAR, Z0, Z0B, CHRNCK)
...
IF (LLGCBZ0) THEN
      ...
      DO IJ = KIJS, KIJL
        ...
        ! IT IS ONLY ADDED VECTORALLY FOR THE SPEED USED TO COMPUTE THE DRAG COEFFICIENT * EFFECTIVE WIND SPEED
        ! BUT THEN IT IS MUTIPLIED BY THE ACTUAL WIND SPEED TO GET THE STRESS
        ! THIS MEANS THAT THE GEOMETRIC MEAN NEEDS TO USED HERE: 
!        XKUEFF = XKAPPA * SQRT( UTOP(IJ) * SQRT(UTOP(IJ)**2 + WSTAR(IJ)**2) )
...
!           CONVERGENCE ?
          ...
          ELSE
            CHRNCK(IJ) = G*SQRT(MAX((Z0(IJ) - HZ0VISO1MX )**2 - HZ0VISO1MX**2, 0.0_JWRB))/MAX(USTAR(IJ),EPSUS)**2
            CHRNCK(IJ) = MAX(CHRNCK(IJ), ALPHAMIN)
          ENDIF

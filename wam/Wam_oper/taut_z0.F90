...
&                  HALP, UTOP, UDIR, WSTAR, TAUW, TAUWDIR, RNFAC, &
&                  USTAR, Z0, Z0B)
...
!       *CALL* *TAUT_Z0(KIJS, KIJL, IUSFG, FL1, WAVNUM, UTOP, UDIR, WSTAR, TAUW, TAUWDIR, RNFAC, USTAR, Z0, Z0B)
...
!          *WSTAR*   - FREE CONVECTION VELOCITY SCALE
!          *TAUW*    - WAVE STRESS.
...
#include "stress_gc.intfb.h"
      ...
      REAL(KIND=JWRB), DIMENSION(KIJS:KIJL), INTENT(IN) :: HALP, UTOP, UDIR, WSTAR, TAUW, TAUWDIR, RNFAC
...
!     CD = ACDLIN + BCDLIN*SQRT(PCHAR) * U10
      ...
      REAL(KIND=JWRB) :: WW, XLOGXL, XKUEFF, XOLOGZ0
      ...
      REAL(KIND=JWRB), DIMENSION(KIJS:KIJL) :: W1, W2
...
IF (LLGCBZ0) THEN
      ...
      DO IJ = KIJS, KIJL
        WW = TANH(10.0_JWRB*(UTOP(IJ)-5.0_JWRB)) + 1.0_JWRB
        W1(IJ) = 0.85_JWRB - 0.05_JWRB*WW
        W2(IJ) = 1.0_JWRB - 0.5_JWRB*WW
      ENDDO

      DO IJ = KIJS, KIJL
        ! ADD CONVECTIVE VELOCITY SCALE IN STRESS CALCULATION AS DONE IN IFS
        ! BUT ONLY FOR LOW WINDS BECAUSE THE GUSTNESS IS ACCOUNTED FOR IN THE WAVE INPUT
        ! FOR LARGER WINDS
        XKUEFF = XKAPPA*(W2(IJ)*SQRT(UTOP(IJ)**2 + WSTAR(IJ)**2) + (1.0_JWRB - W2(IJ))*UTOP(IJ))
...
!!!debile
        write(*,*) 'debile no first convergence ', UTOP(IJ),DEL,USTAR(IJ)
          ...
          Z0(IJ) = MAX(XNLEV/(EXP(XKUEFF/USTAR(IJ))-1.0_JWRB), Z0MINRST)
          Z0B(IJ) = Z0MINRST
        ELSE
          Z0(IJ) = MAX(XNLEV/(EXP(XKUEFF/USTAR(IJ))-1.0_JWRB), Z0MIN)
...
!       Refine solution
        ...
        IF (X < 0.99_JWRB) THEN
          ...
          DO ITER=1,NITER
            ...
            !!!! It is not ideal !!!
            Z0(IJ) = MAX(XNLEV/EXP(XKUEFF/USTOLD), Z0MIN)
...
ELSE
      ...
      DO IJ = KIJS, KIJL
        W2(IJ) = 1.0_JWRB - 0.5_JWRB*( TANH(10.0_JWRB*(UTOP(IJ)-5.0_JWRB)) + 1.0_JWRB )
      ENDDO

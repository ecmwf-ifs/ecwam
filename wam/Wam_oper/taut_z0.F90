...
!     CD = ACDLIN + BCDLIN*SQRT(PCHAR) * U10
      ...
      REAL(KIND=JWRB) :: XLOGXL, XKUEFF, XOLOGZ0
...
!!! not now        XKUEFF = XKAPPA*SQRT(UTOP(IJ)**2 + WSTAR(IJ)**2)
        XKUEFF = XKAPPA*UTOP(IJ)
        USTOLD = USTAR(IJ)
        TAUOLD = USTOLD**2
...
ELSE
      ...
      DO IJ=KIJS,KIJL
        ! ADD CONVECTIVE VELOCUTY SCALE IN STRESS CALCULATION AS DONE IN IFS
! not now        XKUEFF = XKAPPA*SQRT(UTOP(IJ)**2 + WSTAR(IJ)**2)
        XKUEFF = XKAPPA*UTOP(IJ)

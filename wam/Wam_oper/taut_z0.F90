...
#include "stress_gc.intfb.h"
      ...
      REAL(KIND=JWRB), DIMENSION(IJS:IJL) :: ANG_GC
...
IF (LLGCBZ0) THEN
      ...
      CASE(0)
        DO IJ = IJS, IJL
          ANG_GC(IJ) = MAX(0.31_JWRB+0.29_JWRB*TANH(0.18_JWRB*(UTOP(IJ)-10.0_JWRB)),0.15_JWRB)
        ENDDO
      CASE(1) 
        DO IJ = IJS, IJL
          ANG_GC(IJ) = MAX(0.25_JWRB+0.35_JWRB*TANH(0.1_JWRB*(UTOP(IJ)-10.0_JWRB)),0.1_JWRB)
        ENDDO
      END SELECT 
...
!         Z0 IS DERIVED FROM THE NEUTRAL LOG PROFILE: UTOP = (USTAR/XKAPPA)*LOG((XNLEV+Z0)/Z0)
          ...
          CALL STRESS_GC(ANG_GC(IJ), USTAR(IJ), Z0(IJ), HALP(IJ), TAUUNR(IJ))

...
#include "stress_gc.intfb.h"
      ...
      REAL(KIND=JWRB) :: US2TOTAUW, USMAX, ANG_GC, Z0MAX
...
!         CONVERGENCE ?
        ...
        TAUNEW = USTAR(IJ)**2
        USTM1 = 1.0_JWRB/MAX(USTAR(IJ),EPSUS) 
        X = MIN(TAUW(IJ)/TAUNEW,0.9999_JWRB)
        Z0MAX = RNUM*USTM1 + ALPHAOG(IJ)*TAUNEW/SQRT(1.0_JWRB-X)
        Z0(IJ) = MIN(MAX(XNLEV/(EXP(XKUTOP/USTAR(IJ))-1.0_JWRB), Z0MIN), Z0MAX)

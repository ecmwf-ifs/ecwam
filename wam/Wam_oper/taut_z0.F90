SUBROUTINE TAUT_Z0(IJS, IJL, IUSFG, FL1, UTOP, UDIR, ROAIRN, TAUW, TAUWDIR, USTAR, Z0)
...
!       *CALL* *TAUT_Z0(IJS, IJL, IUSFG, FL1, UTOP, UDIR, ROAIRN, TAUW, TAUWDIR, USTAR, Z0)
...
! ----------------------------------------------------------------------
      ...
      USE YOWPHYS  , ONLY : XKAPPA, XNLEV, RNU, RNUM, ALPHA, ALPHAMIN, &
...
#include "stress_gc.intfb.h"
      ...
      REAL(KIND=JWRB), DIMENSION(IJS:IJL), INTENT(IN) :: UTOP, UDIR, ROAIRN, TAUW, TAUWDIR
      ...
      REAL(KIND=JWRB) :: CONST, TAUV, DEL, USTARMIN
      ...
      REAL(KIND=JWRB), DIMENSION(IJS:IJL) :: ANG_GC, ALPHAP, HALP, TAUUNR, ZBREDUC, ZB
...
!!!!        USTAR(IJ) = SQRT(TAUOLD)
      ...
      IF(LLCAPCHNK) THEN
        DO IJ = IJS, IJL
          ZBREDUC(IJ) = SQRT(ANG_GC_F + ANG_GC_G * (1.0_JWRB - TANH(UTOP(IJ)-ANG_GC_H)))
        ENDDO
      ELSE
        DO IJ = IJS, IJL
          ZBREDUC(IJ) = 1.0_JWRB
...
!         CONVERGENCE ?
          ...
          Z0MIN = ALPHAOG(IJ)*TAUOLD 
        ENDDO
        USTARMIN = XKUTOP / LOG(1.0_JWRB + XNLEV/Z0MIN) 
        USTAR(IJ) = MAX(ZBREDUC(IJ)*USTAR(IJ), USTARMIN)
!!        Z0(IJ) = MAX(XNLEV/(EXP(XKUTOP/USTAR(IJ))-1.0_JWRB), Z0MIN)
        Z0(IJ) = XNLEV/(EXP(XKUTOP/USTAR(IJ))-1.0_JWRB)

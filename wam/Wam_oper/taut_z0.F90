...
!     CD = ACDLIN + BCDLIN*SQRT(PCHAR) * U10
      ...
      LOGICAL :: LLSOLVLOG  ! if true, solve the log profile
...
!!!          CDFG = MAX(MIN(0.00008_JWRB*UTOP(IJ), 0.001_JWRB+0.0018_JWRB*EXP(-0.05_JWRB*(UTOP(IJ)-35._JWRB))),0.0005_JWRB)
          X = MIN(TAUWACT(IJ)/USTAR(IJ)**2,0.99_JWRB)
          ZCHAR = MIN( 0.006_JWRB * GM1 * USTAR(IJ)**2 / SQRT(1.0_JWRB - X), 0.05_JWRB*EXP(-0.05_JWRB*(UTOP(IJ)-35._JWRB)) )

...
USE YOWCOUT  , ONLY : JPPFLAG, FFLAG, GFLAG, NFLAG, NIPRMOUT, ITOBOUT, INFOBOUT
...
USE YOWGRID  , ONLY : IJS, IJL, NPROMA_WAM, NCHNK, KIJL4CHNK, IJFROMCHNK
...
!$OMP   PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(ICHNK, KIJS, KIJL, IJSB, IJLB, IR, IJ, IJGLO)
DO ICHNK = 1, NCHNK
  ...
  ! MPI RANK:
  IR = IR + 1
  BOUT(IJSB:IJLB, IR, ICHNK) = REAL(MOD(IRANK,65536), JWRB)

  IR = IR + 1
  BOUT(IJSB:IJLB, IR, ICHNK) = REAL(IRANK/65536, JWRB)


  ! LAND/SEA MASK:
  IR = IR + 1
  BOUT(IJSB:IJLB, IR, ICHNK) = REAL(NLOCMSK(IJSB:IJLB), JWRB)


  ! GLOBAL GRID INDEX (sea and land points):
  IR = IR + 1
  BOUT(IJSB:IJLB, IR, ICHNK) = REAL(MOD(NGLOIND(IJSB:IJLB),65536), JWRB)

  IR = IR + 1
  BOUT(IJSB:IJLB, IR, ICHNK) = REAL(NGLOIND(IJSB:IJLB)/65536, JWRB)


  ! LOCAL SEA POINT INDEX:
  IR = IR + 1
  DO IJ = IJSB, IJLB
    BOUT(IJ, IR, ICHNK) = REAL(MOD(IJ,65536), JWRB)
  ENDDO

  IR = IR + 1
  DO IJ = IJSB, IJLB
    BOUT(IJ, IR, ICHNK) = REAL(IJ/65536, JWRB)
  ENDDO
  ...
  IF ( .NOT. LL1D) THEN
    DO IJGLO = 1, NIBLO
      ...
      IF ( IJ >= IJSB .AND. IJ <= IJLB ) THEN 
         BOUT(IJ, IR, ICHNK) = REAL(MOD(IJGLO,65536), JWRB)
      ENDIF
    ENDDO
  ELSE
    DO IJ = IJSB, IJLB
  ...
  IF ( .NOT. LL1D) THEN
    DO IJGLO = 1, NIBLO
      IJ = IJ2NEWIJ(IJGLO) 
      IF ( IJ >= IJSB .AND. IJ <= IJLB ) THEN 
         BOUT(IJ, IR, ICHNK) = REAL(IJGLO/65536, JWRB)
      ENDIF
    ENDDO
  ELSE
    DO IJ = IJSB, IJLB

      SUBROUTINE STRESS_GC(ANG_GC, USTAR, Z0, ALPHAP, XMSSCG, TAUWCG)
...
!          -------------------------------------------
      ...
      DELK_GC(NS) = 0.5*(XK_GC(NS+1)-XK_GC(NS))/C2OSQRTVG(NS)
      DO I=NS+1,NWAV_GC-1
        DELK_GC(I) = 0.5_JWRB*(XK_GC(I+1)-XK_GC(I-1))/C2OSQRTVG(I)
      ENDDO
      DELK_GC(NWAV_GC) = 0.5_JWRB*(XK_GC(NWAV_GC)-XK_GC(NWAV_GC-1))/C2OSQRTVG(NWAV_GC)

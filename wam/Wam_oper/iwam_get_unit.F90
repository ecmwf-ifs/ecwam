INTEGER(KIND=JWIM) FUNCTION IWAM_GET_UNIT (KUSO, CDNAME, CDACCESS, CDFORM, KRECL)
...
! ----------------------------------------------------------------------
 ...
 2004   FORMAT(A,I3,A,A,A,A,A,A,A,A)
        ...
        IF ( CLACCESS(1:6) == "DIRECT" ) THEN
          OPEN (UNIT=JUME, FILE=CDNAME, FORM=CLFORM, ACCESS=CLACCESS, RECL=KRECL)
        ELSE
          OPEN (UNIT=JUME, FILE=CDNAME, FORM=CLFORM, ACCESS=CLACCESS, POSITION=CLPOSITION)
        ENDIF
      ENDIF

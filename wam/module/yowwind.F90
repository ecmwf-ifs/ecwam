...
!          See *INIT_FIELDG* for the initialisation
      ...
      TYPE FORCING_FIELDS 
        REAL(KIND=JWRB) :: UWND    ! INPUT U COMPONENT ON WAVE MODEL GRID of
                        ! 10m wind, or friction velocity or surfact stress
                        ! See ICODE and ICODE_CPL
        REAL(KIND=JWRB) :: VWND    ! INPUT V COMPONENT ON WAVE MODEL GRID of
                        ! 10m wind, or friction velocity or surfact stress
                        ! See ICODE and ICODE_CPL
        REAL(KIND=JWRB) :: AIRD    ! INPUT AIR DENSITY ON WAVE MODEL GRID
        REAL(KIND=JWRB) :: ZIDL    ! INPUT Zi/L ON WAVE MODEL GRID
        REAL(KIND=JWRB) :: CIFR    ! INPUT SEA ICE FRACTION ON WAVE MODEL GRID 
        REAL(KIND=JWRB) :: CITH    ! INPUT SEA ICE THICKNESS ON WAVE MODEL GRID 
        REAL(KIND=JWRB) :: LKFR    ! INPUT LAKE FRACTION ON WAVE MODEL GRID 
        REAL(KIND=JWRB) :: UCUR    ! INPUT U COMPONENT OF CURRENT ON WAVE MODEL GRID
        REAL(KIND=JWRB) :: VCUR    ! INPUT V COMPONENT OF CURRENT ON WAVE MODEL GRID
        REAL(KIND=JWRB) :: WSWAVE  ! INPUT WIND SPEED (WAVE PARAMETER 245) ON WAVE MODEL GRID
        REAL(KIND=JWRB) :: WDWAVE  ! INPUT WIND DIRECTION (WAVE PARAMETER 249) ON WAVE MODEL GRID
        REAL(KIND=JWRB) :: USTAR   ! INPUT FRICTION VELOCITY 
      ...
      REAL(KIND=JWRB), ALLOCATABLE :: NORMS_IN(:,:)

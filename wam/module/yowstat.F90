...
!                         AND ALTIMETER DATA ASSIMILATION WINDOW.
      ...
      CHARACTER(LEN=256) ::  CFDBSF 
      CHARACTER(LEN=256) ::  CFDB2DSP 
      ...
      INTEGER(KIND=JWIM) :: ISIGHUP 
      INTEGER(KIND=JWIM) :: ISIGINT 
      INTEGER(KIND=JWIM) :: NTASKS 
      INTEGER(KIND=JWIM) :: NSIZE 
      INTEGER(KIND=JWIM) :: NENSFNB 
      INTEGER(KIND=JWIM) :: NTOTENS 
      INTEGER(KIND=JWIM) :: NSYSNB
      INTEGER(KIND=JWIM) :: NMETNB
      INTEGER(KIND=JWIM) :: NWFDBREF 
...
!      *CFDBSF*    CHAR*256  ROOT DIRECTORY FOR SCALAR GRIB FIELDS.
!      *CFDB2DSP*  CHAR*256  ROOT DIRECTORY FOR 2D SPECTRA NON GRIB DATA
...
!      *NTASKS*    INTEGER   NUMBER OF PROCESSORS.
!      *NSIZE*     INTEGER   WORK LOAD PER PROCESSOR.

!      *ISIGHUP*   INTEGER   SIGNAL PRECESSING: SET TO 1 IF hangup (-1) 
!      *ISIGINT*   INTEGER   SIGNAL PRECESSING: SET TO 1 IF interrupt
!                            (-2)

!      *NWFDBREF*  INTEGER   FDB REFERENCE FOR WAVE FIELDS.
!      *LFDBOPEN*  LOGICAL   TRUE IF THE FDB HAS BEEN OPEN FOR WAVE FIELDS.

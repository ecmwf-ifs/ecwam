...
!**********************************************************************
     ...
     &            CICOVERNEXT, CITHICKNEXT, RWFAC
      ...
      IF(.NOT.ALLOCATED(USNEXT)) ALLOCATE(USNEXT(IJS(1):IJL(1),NSTORE,NBLO))
      IF(.NOT.ALLOCATED(DSNEXT)) ALLOCATE(DSNEXT(IJS(1):IJL(1),NSTORE,NBLO))
      IF(.NOT.ALLOCATED(ADSNEXT)) ALLOCATE(ADSNEXT(IJS(1):IJL(1),NSTORE,NBLO))
      IF(.NOT.ALLOCATED(ZIDLNEXT)) ALLOCATE(ZIDLNEXT(IJS(1):IJL(1),NSTORE,NBLO))
      IF(.NOT.ALLOCATED(CICOVERNEXT)) ALLOCATE(CICOVERNEXT(IJS(1):IJL(1),NSTORE,NBLO))
      IF(.NOT.ALLOCATED(CITHICKNEXT)) ALLOCATE(CITHICKNEXT(IJS(1):IJL(1),NSTORE,NBLO))
      !
...
!          VV = VV_wind - RWFAC*V(IJ,IG)
        ...
        !
        USNEXT(IJ,ISTORE,IG)=eWindSpeed
        DSNEXT(IJ,ISTORE,IG)=eWindDir
        ADSNEXT(IJ,ISTORE,IG)=eAIRD
        ZIDLNEXT(IJ,ISTORE,IG)=eZIDL
        CICOVERNEXT(IJ,ISTORE,IG)=eCICOVER
        CITHICKNEXT(IJ,ISTORE,IG)=eCITHICK
      END DO
      END SUBROUTINE
!**********************************************************************
!*                                                                    *
